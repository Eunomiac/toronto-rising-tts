...
2. **Seat-assignment UI refresh** — `refreshGlobalUiAfterSeatAssignment` in `core/global_script.ttslua`:
   ...
   - **First visit only (per player, per seat, since connect):** `revealSeatHudVisibility(seatColor)` — runtime `U.setAttribute(id, "visibility", seatColor)` on that seat’s predeclared Global HUD roots. Then targeted `UpdateUIDisplays`.
...
- Join-stress: enable Defer setXML + Defer Auto-Seat for the joining PC → client stays Grey until Auto-Seat; Refresh XML when ready.

## Related

- [TOR-375](https://linear.app/eunomiac-dev/issue/TOR-375) eliminate runtime full UI refresh
- [TOR-381](https://linear.app/eunomiac-dev/issue/TOR-381) join-client deferred resync fallback
- [TOR-428](https://linear.app/eunomiac-dev/issue/TOR-428) Defer setXML + Auto-Seat controls
