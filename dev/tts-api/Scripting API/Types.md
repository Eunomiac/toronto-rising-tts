...
|---|---|---|---|
|`nil`|nil|No value.|` nil `|
|` int `|int|Non-decimal value.|`5`|
|` float `|float|Non-exact decimal value.|`2.032`|
|` boolean `|bool|` true ` or ` false ` value.|` true `|
|` string `|string|A series of characters.|`"Hello."`|
|` table `|table|A container with keys and values.|`{[ "key" ] = "value", true, 5 }`|
|` vector `|vector|See section [Vector](#vector) for more info.| |
|` color `|color|See section [Color](#color) for more info.| |
|` function `|function|See section [Function](#function) for more info.| |
|` object `|object|An in-game physical Object. Sometimes Global.|` Global or self `|
|` player `|player|An in-game Player. See [Player](../player/instance/) for more info.|`Player [ "White" ]`|
|` variable `|variable|A combination of other types.| |
...
` color ` Color is a type of Table that is used to define RGB values for tinting.
You can use the [Color](../color/)class to manipulate colors.
...
You are also able to use a String in place of a color table. Using a [Player Color](../player/colors/)will automatically fill in that value. It works with any of the 12 color names, as they are written on the [Player Color](../player/colors/)page.
...
` vector ` Vector is a type of Object with x, y, and z coordinates that is used to define a position, rotation or direction.
...
` function ` A function is a section of code that can be run when triggered. In Lua, you are able to pass (use as a parameter) functions. Some elements in Tabletop Simulator can be passed functions to perform some action with, like triggering it after a delay.
