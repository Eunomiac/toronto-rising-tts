...
- Confirm Intermission: global cover comes down together with leftover-audio fade-out and TR_Loop fade-in (~2s), then no-scene table prep under cover, AdminDark. Play: TR_Loop fades ~0.5s; the stacked cover explode starts, then the Music C overture (track + timing from `C.SessionStartAnimationData[sessionNum]`) so the opening drum hits with the first cover scale (TOR-533); game-state overlay and player HUD are armed behind the cover before the explode (TOR-532); the panel hides near the end of that session's song, layers reset, then Main fades in and the Willpower heal overlay can appear
...
Status: current (TOR-143 / TOR-361 / TOR-362 / TOR-497 / TOR-516 / TOR-531 / TOR-532 / TOR-533 / TOR-98)
...
2. `PLAY` — Session start: stacked cover explode first, then Music C overture (TR_Loop 0.5s fade + immediate sting) on the opening drum, OutdoorDim lights under cover, panel hide near sting end, Main playlist, Superficial WP heal + optional broadcast. Contains most gameplay.
...
* **Cover explode then intro sting (TOR-516 / TOR-531 / TOR-533):** Play enter kicks `SessionExplode.play()` first (cover scale-and-fade starts in that call), then `Phases.fireSessionIntro` (TR Loop fades ~0.5s, Music C overture immediately at full volume). That order keeps the opening drum on the first cover scale instead of after Lua/coroutine setup. The catalog key and song length come from `C.SessionStartAnimationData` for the current `sessionNum` (missing index uses `[1]`). Gain is set to catalog volume **before** `playTriggerEffect`; looping silent-arm is skipped. Main mood is **not** started under the sting (`sessionIntroActive` holds reconcile). Pair spacing, waver, and scale times multiply by `songDuration / C.SessionStartBaseDuration`. Lighting still applies under the cover while this runs.
