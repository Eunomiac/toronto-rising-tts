...
* **Cover animation + intro sting:** After lighting, `SessionExplode.playAttribute()` runs the character / number / title splash. TR_Loop starts fading as soon as the splash starts, over the scaled song lead-in (`ATTRIBUTE_SONG_START_DELAY * timeRatio`), and reaches silence when Music C starts. Phases **Quick Transition** (default off) calls `playAttribute(0)` instead: timeRatio 0 (unscalable tail only) and no Music C. Catalog key and song length come from `C.SessionStartAnimationData` for the current `sessionNum` (missing index uses `[1]`). Gain is set to catalog volume **before** `playTriggerEffect`; looping silent-arm is skipped. Main mood is **not** started under the sting (`sessionIntroActive` holds reconcile). No heavy Play-enter lighting or HUD work runs during the animation.
* Play-enter waits `attributeSequenceDurationSec` (catalog length, or the unscalable tail when Quick Transition is on). The sequence hides `overlay_globalBlindfold_panel` near the end and resets layer attrs for the next Intermission. Competing auto-hide from `applyGlobalBlindfoldFromPhase` is suppressed while `Phases.isAdvancing()` (TOR-363). Advance `U.chain` `maxWait` uses `maxPlayEnterWaitSec()` plus 15s so the catalog-length splash is not killed by the default 60s cap (TOR-501 / TOR-531).
