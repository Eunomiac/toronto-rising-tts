...
- Confirm Intermission: global cover comes down together with leftover-audio fade-out and TR_Loop fade-in (~2s), then no-scene table prep under cover, AdminDark. Play: TR_Loop fades ~0.5s with the stacked cover explode; Music C overture starts 0.5s later (`C.SessionStartIntroDelaySec`) so the opening drum hits with the first visible cover scale (TOR-534); game-state overlay and player HUD are armed behind the cover before the explode (TOR-532); the panel hides near the end of that session's song, layers reset, then Main fades in and the Willpower heal overlay can appear
...
2. `PLAY` — Session start: stacked cover explode first, Intermission Loop fades 0.5s, then Music C overture (opening drum delayed 0.5s to match the first visible cover scale), OutdoorDim lights under cover, panel hide near sting end, Main playlist, Superficial WP heal + optional broadcast. Contains most gameplay.
...
* **Cover explode then intro sting (TOR-516 / TOR-531 / TOR-533 / TOR-534):** Play enter kicks `SessionExplode.play()` first (cover scale-and-fade starts in that call) and starts the Intermission Loop fade. After `C.SessionStartIntroDelaySec` (0.5s) it runs `Phases.fireSessionIntro` (Music C overture at full volume). That wait lets the opening drum hit with the first visible cover scale. The catalog key and song length come from `C.SessionStartAnimationData` for the current `sessionNum` (missing index uses `[1]`). Gain is set to catalog volume **before** `playTriggerEffect`; looping silent-arm is skipped. Main mood is **not** started under the sting (`sessionIntroActive` holds reconcile). Pair spacing, waver, and scale times multiply by `songDuration / C.SessionStartBaseDuration`. Lighting still applies under the cover while this runs.
