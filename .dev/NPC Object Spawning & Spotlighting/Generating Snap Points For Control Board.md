...
- Every snap uses `rotation_snap = true` and tags `{ npc_control_token, pc_control_token }` (TTS OR match), plus board-local yaw **toward the master/default `origin`** plus ring-level or default `snapYawOffsetDeg`.
