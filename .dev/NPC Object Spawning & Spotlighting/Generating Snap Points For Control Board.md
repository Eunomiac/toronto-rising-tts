...
- **Unit:** **STAGE_BOARD / playfield world XZ inches**, then mapped onto **CONTROL_BOARD u/v**. The painted minimap tile is **2:1** (`CONTROL_BOARD` scale 20×10). Live `STAGE_BOARD` Z was scaled independently (~800×289), which is why minimap table markers use different X vs Z divisors (`Snaps.minimapScaleRatios`). Dashboard stagger conversion uses that **control-tile aspect** (implied stage depth = `controlZ × stageX/controlX` ≈ 400), not live `scaleZ`, so neighbor snaps stay on the painted holes. Anchor snaps (`familyK == 0`) never use stagger.
