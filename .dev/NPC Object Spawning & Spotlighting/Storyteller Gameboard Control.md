...
| **Clear** | First click: recover stray tokens (not on CONTROL_BOARD or palette) to palette parking; arm 5s confirm. Second click → empty `placements`, `Gameboard.syncTokensToPalette()` (group layout from `npcs_data`), `Sync.npcs` |
