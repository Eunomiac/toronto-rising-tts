...
| Home | palette group slot / polar placement / homeland numbered chair (`tableSlot`) | numbered chair from `seatSlots[color].tableSlot`; Absent tokens are locked at world Y = −200 |
...
| Reconcile pin | placement rows + homeland chair matching `tableSlot` | token pinned to that color's `tableSlot` chair (or locked at Y = −200 if Absent); flip matches `seatSlots[color].isPresent` |
...
**Scope of the current contract (Apply-time occupancy + mirror):** Each seat-row snap is a numbered table chair (`tableSlot`). Token identity is the occupant. HERE Apply reads every PC and NPC token on those snaps and writes `sessionScene.seatSlots` (`tableSlot` / `absentFromSession` / `isPresent` from flip). A PC token that is not on any chair snap is Absent (pile under the table). Reconcile pins tokens to match that state: in-session PCs on their chair, Absent PCs **locked at world Y = −200** (disabled; not on a chair, so the next Apply does not re-seat them). Cover transitions onto Table B shuffle packed occupancy and then remirror those tokens (TOR-537); Absent tokens stay stashed. Two tokens on one snap fail loud and abort Apply. Slot number fields are not on the PCs or Scenes panels — move tokens, then Apply. The PCs panel **Absent** toggle remains and stashes the matching token. Play-as-NPC role swap is **TOR-95**.
...
**Palette vs activated:** Tokens on **CONTROL_BOARD_PALETTE** are inactive storage; **Apply** ignores them until a token is on a CONTROL_BOARD polar snap. **Clear** runs `Palette.syncTokensToPalette` (group order from `lib/npcs_data` `characters[].groups`, minus `PALETTE_GROUP_BLACKLIST` e.g. `princesCourt`). **On-board detection:** Standard drop handlers and scans use `isTokenOnControlBoardSurface` (2.5× tile XZ, no Y — snap settle / seat scale). Scatter drop uses the tighter `isTokenOnMinimapTokenSurface` (1.05× XZ + surface Y, palette excluded). Scatter calibrate / Apply use `Board.objectsAboveControlBoard` (`U.findAboveObject` box cast) plus that same tight surface fallback — not the loose 2.5× band, so palette tokens are not treated as on the parchment. Clear parking and stray recovery use `isTokenOnControlBoardFace` (same XZ band + surface Y) so tokens fallen below the board are parked.
