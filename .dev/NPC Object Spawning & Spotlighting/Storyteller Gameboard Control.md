...
Two distinct control-token tags share the CONTROL_BOARD minimap. Minimap snaps are tagged **`npc_control_token`** and **`pc_control_token`** (TTS OR match — an object needs one of those tags to snap). Only NPC/PC control tokens are expected near the board. The two token kinds drive **different** state and are handled by **disjoint** code paths. A `pc_control_token` must carry **only** `pc_control_token` (never also `npc_control_token`), so the NPC handlers — all gated on `isNpcControlToken` (tag `npc_control_token`) — skip it automatically.
