...
| `EffectiveStats.forPlayer(playerID)` | One resolve pass; `trackerMax`, `dotRating`, `bloodPotencyDerived`, etc. |
| `EffectiveStats.forSeat(color)` | Seat-color convenience for roll paths |
...
**Debug:** `DEBUG.printTable(value, label?)`, `DEBUG.dumpConditions(seatColor)`, `DEBUG.dumpRollPolicy(seatColor)`
...
| `Soundscape.applyContext(context)` | Apply weather/site/calendar audio context; optional `appliedSiteKey` updates `soundscape.lastAppliedSiteKey`. `isSilent = true` mutes ambient lanes only (featured unchanged); `isSilent = false` clears that latch. `locationMusic`/`musicMood` `"none"` **suppresses** background music (site playlist none, e.g. Ravenwing). If both are non-none playlists, **only `musicMood` is applied** (one background crossfade). | `Soundscape.applyContext({ isIndoors = true, weather = "lightRain" })`, `Soundscape.applyContext({ isSilent = true })` |
...
| `M.manualRunPlayerConnect(color)` | Manual Connect: force-run `onPlayerConnect` for that PC (bypasses Defer Connect) | ST PCs panel (TOR-430) |
...
- `C.LightModes` - Scene lighting presets (`IndoorBright`, `OutdoorDim`, …). Chronicle location presets (`Indoor*` / `Outdoor*` / `Underground*` / `Memoriam*`) use `ambient_intensity = 0`, `ambient_type = 1`, and transfer fill to directional overhead via `light_intensity` plus `setLightColor` (amber / cool moonlight / pale green tints via `chronicleOverheadPreset` in `lib/constants.ttslua`). Table spotlights use explicit `spotlights` maps via `chronicleSpotlights`. Darkness is expressed by low `light_intensity` and/or `"OFF"` spotlight entries — preset key suffixes (e.g. `IndoorDark`) do not auto-disable seat lights. When `sessionScene.siteKey` is absent, reconcilers apply `C.DefaultNoLocationLightingPresetKey` (`OutdoorDim`) via `Scenes.syncSessionLightingPresetForLocation` / `Scenes.resolveLightingPresetFromState`.
