...
- `C.LightModes` - Scene lighting presets (`IndoorBright`, `OutdoorDim`, …). Chronicle presets use `ambient_type = 2` with full `gradientColor` (`sky` / `equator` / `ground`) and explicit `spotlights` maps via `chronicleGradientPreset` / `chronicleSpotlights` in `lib/constants.ttslua`. Gradient stops are passed through `liftAmbientColor` (50% toward white, `AMBIENT_GRADIENT_LIFT`) so very dark RGB tints still respond to `ambient_intensity`. Darkness is expressed by low `ambient_intensity` and/or `"OFF"` spotlight entries — preset key suffixes (e.g. `IndoorDark`) do not auto-disable seat lights. When `sessionScene.siteKey` is absent, reconcilers apply `C.DefaultNoLocationLightingPresetKey` (`OutdoorDim`) via `Scenes.syncSessionLightingPresetForLocation` / `Scenes.resolveLightingPresetFromState`.
