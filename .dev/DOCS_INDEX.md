...
| Storyteller dashboard | [`storyteller-dashboard/README.md`](storyteller-dashboard/README.md), [`storyteller-dashboard/PLAYWRIGHT.md`](storyteller-dashboard/PLAYWRIGHT.md) (Cursor Chrome + Playwright MCP), [`storyteller-dashboard/TESTING.md`](storyteller-dashboard/TESTING.md), [`Storyteller Dashboard Docs/Generic NPCs.md`](Storyteller%20Dashboard%20Docs/Generic%20NPCs.md), [`Storyteller Dashboard Docs/Scene Import Guide.md`](Storyteller%20Dashboard%20Docs/Scene%20Import%20Guide.md), [`../.cursor/rules/toronto-rising-dashboard.mdc`](../.cursor/rules/toronto-rising-dashboard.mdc) | Second-monitor **Storyteller-only** web app (never a player HUD). Same git/GitHub repo as TTS Lua; dashboard-only tracking is [`storyteller-dashboard/agent/Running Tasklist.md`](storyteller-dashboard/agent/Running%20Tasklist.md). Stack: Vite + React + Node on port 8788. Generic NPC rows refresh from **Generics Export** on dashboard server startup. Scenes catalogs: `npm run dashboard:scene-catalogs` (also dashboard `dev`/`build`); polar stagger reads STAGE_BOARD from the live TTS save. Scene Import Guide is the live import JSON contract. Playwright must attach to Chrome profile **Cursor** (`Profile 3`) via `--extension`, not a private `--user-data-dir` window. |
