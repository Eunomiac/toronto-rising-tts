...
| `HUD_memoriamModalPcDropdown` | `memoriam_modal_pc_selection` | `(player, value, id)` | ST-gated. Builds that PC's timeline bar, date slider, and scene grid (`core/memoriam_modal.ttslua`). |
| `HUD_memoriamModalSlider` | `memoriam_modal_period_selection_slider` | `(player, value, id)` | ST-gated. Maps 0 (earliest, left) – 2400 (present, right) onto the timeline (date, location, gold bar including nested chunks, scene highlight). Deselects a scene button that is not in the new period. |
| `HUD_memoriamModalSceneButton` | `memoriam_period_*_panel*_button` | `(player, button, id)` | ST-gated. Stores Memoriam destination panel; if that column is not highlighted, jumps the slider to a random exclusive date in that period. Shows PC-as-NPC rows (subject hidden). |
...
| `HUD_memoriamModalNpcPlus` / `NpcPick` / `NpcInput` / `NpcConfirm` / `NpcCancel` | assignment `+`, picker buttons, NPC key field, OK / Cancel | clicker / `(player, value, id)` | ST-gated. Picker overlay for mapping other PCs onto period NPCs or a library key. |
