...
| `HUD_debugSeatColor` | `debugSeat_Black`, `debugSeat_Red`, `debugSeat_Orange`, `debugSeat_Brown`, `debugSeat_Pink`, `debugSeat_Purple` | `(player, button, id)` | Bottom-right debug row (left of admin twirl-down). **Left-click (`"-1"`):** `player.changeColor(<Color>)`, then `hideStartupLoadingOverlays()`. **Right-click (`"-2"`):** `M.setCamera(player, "default<Color>")` (e.g. `defaultBrown`). Active Host seat button gets a **red** outline (`#FF0000`, 4px) via `syncDebugSeatColorButtons()` (`onPlayerChangeColor`, `UpdateUIDisplays`, after left-click). Seat changes also run **`refreshGlobalUiAfterSeatAssignment`** — full `UI.setXml` once per player per seat since connect; repeat swaps use `UpdateUIDisplays` only (TOR-285). Post-refresh: clear orphaned `hudBlindfold` on empty PC seats + invalidate overlay `lastVisible` caches so XmlUI reset cannot leave a stuck blindfold (TOR-324). |
...
| `HUD_scenesLibSlot` | `scenes_lib_slot_01` … `scenes_lib_slot_20` | `(player, button, id)` | Host: sets `sceneLibrary.activeKey` from `order[slot]`; resets clock draft to activation preview; panel refresh previews Table/Seats/Location/Time from that row when pending (blue “on” colors). Does not apply live state. |
