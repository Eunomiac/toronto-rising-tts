...
| Handler | XML Element(s) | Params | Behavior |
| ------- | ---------------- | ------ | -------- |
| `HUD_selectStorytellerPanel` | `toggle_scenes`, `toggle_soundscape`, `toggle_pcs`, `toggle_phases`, `toggle_stats` | `(player, button, id)` | Strips `toggle_` prefix from `id`, calls `StorytellerPanelUI.selectStorytellerPanel(panelKey)` to show one storyteller panel and hide all others. Updates toggle button colors to indicate active panel. Re-clicking the open tab (or `closeAllPanels`) also collapses `storytellerToolbarBody`; switching panels does not (TOR-395). Deferred refresh for Sound (`syncSoundscapeControls`), Scenes (`StorytellerScenesPanel.refresh` + lighting preset buttons), and Stats (`StorytellerStatsPanel.refresh`). |
...
| `HUD_scenesApplyLocation` | `Apply location + soundscape` | `(player, button, id)` | Reads panel display district/site (pending library row or live). **Pending:** validates + writes keys/top-fog onto selected library row only (no soundscape). **Live:** validates, writes live keys + `Soundscape.applyContext` + hosted reconcile / sheets. |
...
| `HUD_phaseArmJoinXml` | `phase_armJoinXml` | `(player, button, id)` | ST-gated. Sets `joinXmlArmed` + Defer setXML; remounts **minimal** Global XmlUI before a struggling client joins (TOR-439 experiment). `joinXmlArmed` persists in save state; **onLoad** remounts minimal when armed. For Loading-bar CustomUIAssets tests: File → Save after Arm, or `npm run tts-save:inject-join-minimal` — not Save & Play (rewrites full XmlUI). |
