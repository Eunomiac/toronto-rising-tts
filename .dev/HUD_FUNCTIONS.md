...
| `HUD_clearLoadingOverlay` | `Clear Loading Overlay` button | `(player, button, id)` | ST-gated. Calls `hideStartupLoadingOverlays()` — `UI.hide` **and** `active=false` on global `overlay_loadingScreen_startup` so TOR-285 `UI.setXml` seat refresh cannot resurrect it. Automatic hide on load uses **`U.RunSequence`** in **`core/global_script.ttslua`** `onLoad` (gates + wall-clock delay). Do not use **`U.waitForCondition`** with nested **`U.delay`** for that path — TTS can fire the timer immediately. |
