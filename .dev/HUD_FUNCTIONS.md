...
| `HUD_scenesOpenSiteModal` | `Browse sites...` | `(player, button, id)` | Calls `StorytellerScenesPanel.applySiteModalDistrictVisibility()` then shows `scenes_modal_sites_root`. Modal XML has `scenes_site_group_dist_<districtKey>` panels (inactive except matching trimmed `scenes_districtKey`) plus always-visible `scenes_site_group_generic`. |
