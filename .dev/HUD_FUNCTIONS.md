...
| `HUD_debugLightActivate` | `Debug Light` button | `(player, button, id)` | ST-gated. Opens `panel_debug_lights` on the selection grid and fills the 60-slot pool from in-world `Spotlight` objects (skips NPC preload and the focal marker). |
