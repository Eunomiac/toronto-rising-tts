...
| `HUD_clearLoadingOverlay` | `Clear Loading Overlay` button | `(player, button, id)` | Calls `hideStartupLoadingOverlays()` to force-hide global `overlay_loadingScreen_startup` (debug/manual escape hatch). Automatic hide on load uses **`U.RunSequence`** in **`core/global_script.ttslua`** `onLoad` (gates + wall-clock delay). Do not use **`U.waitForCondition`** with nested **`U.delay`** for that path — TTS can fire the timer immediately. |
