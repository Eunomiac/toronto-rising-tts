...
| `HUD_scenesDaysleepRefresh` | `scenes_daysleep_refresh` (`Refresh`, gold) | `(player, button, id)` | ST: live scene only. Runs the session-start Superficial Willpower heal (`Phases.runSessionStartWillpowerHeal`) — Health is not healed (TOR-482). Stay-gold when live; grey when no live scene. |
