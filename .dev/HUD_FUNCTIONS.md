...
| `HUD_scenesPanelInput` | district/site/clock/npc note fields | `(player, value, id)` | Intentional no-op on keystroke; use Apply buttons to persist. **Scenes location and clock rows are `InputField`s (type to edit, then Apply) — not modal pickers or toggle buttons.** |
