...
| `HUD_scenesOpenSkyboxModal` | `Skyboxes...` / `scenes_openSkyboxModal` | `(player, button, id)` | Left-click: shows `scenes_modal_skyboxes_root` (generated from `C.Skyboxes` + leading **Generic**). **Right-click** (`button == "-2"`): clears `sessionScene.skyboxOverride` (pending library row or live) — site default returns; no `Sync.full` until Apply location. |
