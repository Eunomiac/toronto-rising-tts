...
| `HUD_pcsAbsentFromSession` | `pcs_absent_<Color>` | `(player, value, id)` | ST-gated. Marks a PC seat Absent (no numbered table chair). Turning it off claims the lowest free slot, or stays Absent if the table is full (TOR-507). |
| `HUD_pcsTableSlot` | `pcs_tableSlot_<Color>` | `(player, value, id)` | ST-gated. Sets `sessionScene.seatSlots[color].tableSlot` and re-lays out the table (TOR-507). |
| `HUD_scenesTableSlot` | `scenes_tableSlot_NPC#` | `(player, value, id)` | ST-gated. Sets numbered tableSlot for an NPC identity on Seat Activation (TOR-507). |
