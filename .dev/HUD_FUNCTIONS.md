...
| `HUD_debugSeatColor` | `debugSeat_Black`, `debugSeat_Red`, `debugSeat_Orange`, `debugSeat_Brown`, `debugSeat_Pink`, `debugSeat_Purple` | `(player, button, id)` | Bottom-right debug row (left of admin twirl-down). **Left-click (`"-1"`):** `player.changeColor(<Color>)`, then `hideStartupLoadingOverlays()`. **Right-click (`"-2"`):** `M.setCamera(player, "default<Color>")` (e.g. `defaultBrown`). Changing into a PC seat copies that seat's `cameraAngles` onto the Storyteller; table/scene/Scatter layout recopies the **just-computed** seat cameras using the live TTS seat color (TOR-603 / TOR-605). Active Host seat button gets a **red** outline (`#FF0000`, 4px) via `syncDebugSeatColorButtons()` (`onPlayerChangeColor`, `UpdateUIDisplays`, after left-click). Host hotseat swaps do **not** auto-run `refreshGlobalUiAfterSeatAssignment` — use **`HUD_refreshUi`**. |
