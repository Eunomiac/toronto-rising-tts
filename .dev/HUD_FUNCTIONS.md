...
| `HUD_clearLoadingOverlay` | `Clear Loading Overlay` button | `(player, button, id)` | ST-gated. Calls `hideStartupLoadingOverlays()` — `U.RunSequence`: `UI.hide` (FadeOut), return `1` (1s wait), then `active=false` on global `overlay_loadingScreen_startup` so TOR-285 seat XML refresh cannot resurrect it. Automatic hide on load uses the same helper. Do not use **`U.waitForCondition`** with nested **`U.delay`** for that path — TTS can fire the timer immediately. |
...
| `HUD_scenesLibSlot` | `scenes_lib_slot_01` … `scenes_lib_slot_20` | `(player, button, id)` | Host: left-click sets `sceneLibrary.activeKey` from `order[slot]` (pending preview when ≠ `lastAppliedKey`, blue “on” colors). **Right-click** (`button == "-2"`) on the pending selected row clears preview (`clearLibraryPreviewSelection`) and restores live / no-scene panel controls (green). Does not apply live state. |
