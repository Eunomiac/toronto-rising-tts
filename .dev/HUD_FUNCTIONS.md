...
| `HUD_popoutCameraControl_click` | `popout_cameraPanel_<Color>` | `(player, value, id)` | Left-click opens the picker and `Main.setCamera(player, "default")` (ThirdPerson). Right-click (`-2`) runs the stage look (`M.lookAtNPC`) then FirstPerson after a short settle. Closing the picker resets camera `alpha_control` tints on that seat. |
