...
| `HUD_scenesLibApply` | `scenes_lib_btn_apply_clock`, `scenes_lib_btn_apply_x5`, `scenes_lib_btn_apply_set_present`, `scenes_lib_btn_apply_present` | `(player, button, id)` | ST: id → clock mode (`scene` / `x5` / `setPresent` / `present`). Same staged blindfold Apply for all four; they differ only in clock handling (TOR-142). Scene Time / ×5 / SET Now require authored library datetime; NOW always allowed. Work phase uses `Sync.full({ skipSoundscape = true })`; fade-in owns soundscape. |
