...
| P1-4 | `seatLayout.virtualHandZoneAnchors` is read/written but absent from defaults (formerly also absent from save whitelist). | `lib/rotational-seat-layout.ttslua` reads/writes it. | **Partial (TOR-433):** fail-open `buildSaveState` now persists any safe top-level/`seatLayout` fields including anchors. Still missing from defaults — add `seatLayout.virtualHandZoneAnchors = {}` or move to module-private cache. | Decide ownership: add to defaults, or move off `gameState`. | Rotational seat layout, virtual hand-zone movement, load recovery after table swaps. |
| P1-5 | `npcs.ui.groupExpanded` is stored under `gameState.npcs.ui` but defaults only declare `npcs.instances` (formerly save only wrote instances). | `core/npcs.ttslua` reads/writes `npcs.ui`. | **Partial (TOR-433):** fail-open save now clones full safe `npcs` (including `ui` if present). Defaults still omit `ui`. | Add `npcs.ui = { groupExpanded = {} }` to defaults, or move expansion state to module-private UI cache. | Storyteller NPC panel UX, save/load consistency. |
...
2. Update `.dev/HUD_FUNCTIONS.md` scene authority text.
3. Update `ui/storyteller/panel_scenes.xml` copy for scene lighting and seat presence.
4. Update `.dev/Dice System/Dice System Outline.md` nested stat examples to `S.getPlayerID` + `S.setStateVal`.
5. Update `.dev/SOLVING ISSUES & DEBUGGING.md` scene/seat-presence bullets.
6. Keep `.dev/AVAILABLE_FUNCTIONS.md` canonical patterns current whenever state APIs move.
