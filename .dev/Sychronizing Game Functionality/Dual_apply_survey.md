...
| **Global onLoad / bootstrap** | saved `gameState` slices | early soundscape hard mute, scene/soundscape/lighting/NPC/UI reconcilers; **one** `NPCS.flushDeferredSeatLayoutCommit` at startup gate | `global_script.onLoad` chunk silence, initial `Sync.full({ deferLayoutCommit })`, startup-gate scene state + `Sync.full` + layout flush | Same `Sync.full` pipeline without early `R.SyncTable`; single forced geometry pass while blindfolds down | **2026-07:** seat geometry deferred until startup readiness gate; no 0.5s `R.SyncTable` | Soundscape fingerprint + early silence; lighting epoch + deferred retry list; UI/overlay idempotence | **P2** |
