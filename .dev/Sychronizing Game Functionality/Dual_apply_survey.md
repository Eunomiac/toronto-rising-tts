...
| **Chronicle weather** | `soundscape.weather`, `rain`, `wind`, `thunderEnabled`, `isIndoors` | Rain/wind emitters, thunder scheduler, outdoor rain particle looping index on `G.GUIDS.PARTICLES_WEATHER_RAIN` | `ChronicleWeather.applyScheduledWeather` from scene library apply, Apply clock, realtime clock tick; weather setters (`setRainLayer` / `setWindLayer` / `setThunderEnabled` / `setIndoors`) | Next `Soundscape.reconcileFromState` via `Sync.full` | OK: weather audio still primes fingerprint; rain particles skip `playLoopingEffect` when `getLoopingEffectIndex` already matches (no second weather clock) | On full success: `markReconciledToCurrentState`; on partial failure: `invalidateReconcileCache`; particles follow weather setters only | **P2** |
...
| **Lighting (seat presentation)** | `gameState.lights`, `sessionScene.lightingSeatSpotlightPreset`, `sessionScene.seatPresent`, `seatLayout.*`, player hunger/rolling/conditions | `L.SetLightMode`, Unity light component setters through `L.SetLightMode` | Storyteller seat toggle, `RSL.SyncTable` (visibility only for NPC homeland chairs — no eager NPC seat light writes per TOR-265), NPC Step Four call `L.reconcileForPlayer` | `Sync.full` → seat presentation → `L.reconcileAllPlayers`; `Sync.player` → `L.reconcileForPlayer` | Mitigated: `lastReconciledModeByRef` skips redundant `SetLightMode` when desired mode unchanged | `transitionEpoch` for stale steps; `lastReconciledModeByRef` for duplicate reconcile passes | **P2** |
| **NPCs (session scene layout)** | `npcs.instances`, `sessionScene.npcWorld`, `seatLayout.occupiedNPCSlots`, `gameState.lights[npcLight_*]` | figurine/light moves, NPC spotlight `L.reconcileLightRef`, `RSL.SyncTable` queue | NPC Storyteller panel move helpers; scene-library apply writes `sessionScene` then `Sync.full` | `NPCS.reconcileSessionSceneNpcWorldFromState` in `Sync.full` | Mitigated: workshop figurine pool; audit on load; adopt-only repair on failed move | Global preload pool is workshop objects; panel helpers move pool instances only | **P2** |
| **Overlays / HUD** | `playerData`, overlay flags, active scene/phase summaries | `HO.reconcileForSeat`, `HUDP.updatePlayerUI`, `UI.*` | `Sync.player`, `UpdateUIDisplays`, Storyteller scene seat toggle | `Sync.ui` / `UpdateUIDisplays`; `Sync.full` calls `HO.syncAll` before narrow UI delta | Mitigated for `Sync.player`: per-seat overlay + `UpdateUIDisplays({ playerStats, colors })` only | Incremental `Sync.full` omits overlays after `HO.syncAll`; use `delta.colors` for seat-scoped stats/HUD | **P2** |
...
3. **NPC scene-layout:** workshop pool + audit on load; adopt-only `ensureNpcInPreloadZone`; fingerprint not committed when placement fails.

**Implemented (performance audit, 2026-05-19):**

4. **`Sync.player` / overlay scope:** `HO.reconcileForSeat`, `UpdateUIDisplays.colors`.
