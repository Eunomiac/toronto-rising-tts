...
| `GameStateOverlay.reconcileFromState()` | `UpdateUIDisplays` when full, `gameStateOverlay`, `phase`, or `scenesPanel`; clock tick at zero speed | `gameState.currentPhase`, `gameState.playSubPhase`, `gameState.sessionStartDowntime`, `gameState.sessionName`, `gameState.spotlight`, `gameState.sessionScene.districtKey`, `gameState.sessionScene.siteKey`, `gameState.sessionScene.clock`, `C.Sites`, `C.Districts` | Center-top overlay `UI.setValue` and `UI.setAttribute` (Play clock; session-start prologue copy; Spotlight ritual copy + chrome; End bookend copy + EPILOGUE) | Confirmed no state writes | `overlayTextCache`, `lastDistrictSeparatorFingerprint`, `lastOverlayChromeFingerprint`; hides/clears outside Play / Spotlight / End |
