...
| Storyteller scene library Apply | Replace `gameState.sessionScene` / linked scene state through the scene-library panel path | Close panel + `HUDBF.beginTransition` immediately; `HUDBF.runTransitionAfterLeadIn` defers heavy work 2s via `U.await`, then state write, `RSL.SetTableTo(..., { skipTransitionBlindfold = true })`, soundscape, `Sync.full`; at settle-delay start, `M.setCamera(..., "default")` for seated players; 10s settle before single blindfold lift | Parent Panel `UI.show` once (cache-aware, TOR-431); camera reset is not during table geometry. |
