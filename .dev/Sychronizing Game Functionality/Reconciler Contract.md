...
8. `Spotlight.reconcileFromState()` — after seat layout so hide-list (bags / companions / compulsion decks) wins over layout restore while `currentPhase` is Spotlight or End; seat figurines stay hidden only in Spotlight (TOR-98).
...
| `Scenes.reconcileTopFogFromState(opts)` | `Sync.full`, `Sync.lighting`, bootstrap seat retries | `gameState.sessionScene.isTopFogActive`, `G.GUIDS.TOP_FOG` | `TOP_FOG` AssetBundle `playLoopingEffect` — Effect 2 / index 1 on, Effect 1 / index 0 off | Confirmed no state writes | `lastTopFogReconcileFingerprint`; `{ force = true }`; `Scenes.invalidateReconcileCache()` |
| `Scenes.reconcileSkyboxFromState(opts)` | `Sync.full`, `Sync.lighting` | `gameState.sessionScene.skyboxOverride` (catalog key or `"Generic"`), `gameState.sessionScene.siteKey`, `C.Skyboxes`, `C.Sites[*].skybox`, `C.GenericSkyboxes` | TTS `Backgrounds.setCustomURL` (override key → `C.Skyboxes[key].url`; `"Generic"` → random generic; else site key → URL or random generic) | Confirmed no state writes | `lastSkyboxReconcileFingerprint` (`override:<key>` or `site:<siteKey>`); `{ force = true }`; `Scenes.invalidateReconcileCache()` |
...
| `Spotlight.reconcileFromState()` | `Sync.full` after `NPCS.reconcileAllFromState`; Spotlight enter/exit work | `gameState.currentPhase`, `gameState.spotlight.order`, `gameState.spotlight.frontIndex`, `G.GUIDS.SPOTLIGHT_FIGURE_*` / `SPOTLIGHT_LIGHT_*` | Carousel snap/park, stage lights, Host strip, playfield hide-list (toys parked in Spotlight and End) | Confirmed no state writes except clamping `frontIndex` when order shrinks | `lastCarouselFingerprint` / `lastPlayfieldFingerprint` / `lastControlsFingerprint`; re-parks toys every pass while Spotlight/End; skips carousel snap while `rotating`; `Spotlight.invalidateReconcileCache()` |
...
| `GameStateOverlay.reconcileFromState()` | `UpdateUIDisplays` when full, `gameStateOverlay`, `phase`, or `scenesPanel`; clock tick at zero speed | `gameState.currentPhase`, `gameState.playSubPhase`, `gameState.sessionStartDowntime`, `gameState.sessionName`, `gameState.spotlight`, `gameState.sessionScene.districtKey`, `gameState.sessionScene.siteKey`, `gameState.sessionScene.clock`, `C.Sites`, `C.Districts` | Center-top overlay absolute `UI.setClass` chrome (PLAY/MAIN baseline; Downtime / prologue / Spotlight APPLY) plus `setAttributes` text/active (Play clock; prologue/EPILOGUE bookend; Spotlight ritual copy) | Confirmed no state writes | `overlayTextCache`, `overlayClassCache`, `lastDistrictSeparatorFingerprint`, `lastOverlayChromeFingerprint`; hides/clears outside Play / Spotlight / End |
