...
| `Scenes.reconcileSkyboxFromState(opts)` | `Sync.full`, `Sync.lighting` | `gameState.sessionScene.skyboxOverride`, `gameState.sessionScene.siteKey`, `C.Sites`, `C.GenericSkyboxes` | TTS `Backgrounds.setCustomURL` (`skyboxOverride` when set; else site `skyboxURL` or random generic) | Confirmed no state writes | `lastSkyboxReconcileFingerprint` (override URL or trimmed `siteKey`); `{ force = true }`; `Scenes.invalidateReconcileCache()` |
