...
| `HUD_memoriamModalPcButton` / `HUD_memoriamModalSlider` / `HUD_memoriamModalSceneButton` / `HUD_memoriamModalJustSmoke` / `HUD_memoriamModalNpcPlus` / `HUD_memoriamModalNpcPick` / `HUD_memoriamModalNpcInput` / `HUD_memoriamModalNpcConfirm` / `HUD_memoriamModalNpcCancel` | A | Yes | TOR-539 / TOR-540 / TOR-550 Memoriam popup draft (clicker, ST-gated). Slider is not a hot path. |
