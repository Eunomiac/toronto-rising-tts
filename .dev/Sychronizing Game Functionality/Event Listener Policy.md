...
| `GlobalDashboardPcSheetApply` | B+C | — | Dashboard execute-lua (`dashboard.pc_sheet`): one typed mutation **or a JSON array of mutations** (trackers, dots, disabled, ST badges, Hunger/XP, Desire, Absent/join, ST rolls, **`mergeSeat`** absolute write after client deep-merge) then one snapshot. Damage uses V5 overflow (super→agg). Stain add is a no-op while impaired. No Steam gate. Reuses `PCST` / `P` / `Sync.player` / `FSL.setPlayerAbsentFromSession` / `RC.initiateRoll`. |
...
| `HUD_scenesPanel` / apply / lib / ctor / clock / location+skybox modals / clock lerp | B+C | Yes | modal opens Tier A; TOR-142/TOR-401: Apply button ids → clock modes via `HUD_scenesLibApply` (incl. `presentPlus15`–`120`); TOR-222: `HUD_scenesClockLerpDelta` / `Sun` / `YearGo` animate live clock (display-only mid-lerp; settle = tryAdvance on forward + weather + Sync); TOR-482: `HUD_scenesDaysleepRefresh` ST-gated WP heal (no live scene required; overlay auto-hides); TOR-488: live Apply Location is staged blindfold (fadeIn owns soundscape); skybox pick/clear Tier B until Apply location; TOR-573: `scenes_tbl_Scatter` is a table-change (host, stash/restore Standard occupancy, no new listener). Scatter CONTROL_BOARD `reload()` waits for mesh-ready before snaps/XmlUI so the cover `U.chain` does not C# NRE. |
...
| `HUD_phaseAdvance` / `HUD_setPlaySubPhase` / `HUD_sessionNumInput` / `HUD_sessionNameInput` / `HUD_phaseSessionStartQuick` | B+C | Yes | TOR-143 phase Advance + Play subphases + sessionNum; TOR-98 sessionName; Quick Transition skip Music C + `playAttribute(0)`; TOR-561 Intermission cover rewrite. Memoriam click opens modal (TOR-539). Main/Downtime resolve to live-scene Main vs Downtime, or no-op (TOR-527). Main/Downtime while Memoriam active → `Memoriam.applyExit` restore (TOR-101). TOR-578: first Advance click arms a 5s XmlUI hint (`phase_AdvanceConfirmHint`); second click while armed Advances. Panel close / timeout disarms. |
...
| `HUD_scatterModeControl_selectorHoverOn` / `_selectorHoverOff` | A | — | TOR-602 selector hover: other groups show/hide overlay; occupied group keeps hover image and pulses color to half alpha |
...
| `Gameboard.tryNpcControlTokenDroppedOnStorytellerDiceBag` | `core/npc_gameboard_interactions.ttslua` (via facade) | **Pass** | Black/ST + `dieKindNearStorytellerDiceBag` before restore/roll. Scatter Mode: `STR.initiateFromBagLabel` may replace the same group’s uncleared roll or steal the oldest tray instead of refusing when all three trays are busy. |
