...
| `GlobalDashboardPcSheetApply` | B+C | — | Dashboard execute-lua (`dashboard.pc_sheet`): one typed mutation **or a JSON array of mutations** (trackers, dots, disabled, ST badges, Hunger/XP, Desire, Absent/join, ST rolls, **`mergeSeat`** legacy sheet-shaped write, **`mergePlayerData`** deep-merge into `gameState.playerData.<pid>` for the JSON modal) then one snapshot. Damage uses V5 overflow (super→agg). Stain add is a no-op while impaired. No Steam gate. Reuses `PCST` / `P` / `Sync.player` / `FSL.setPlayerAbsentFromSession` / `RC.initiateRoll`. |
