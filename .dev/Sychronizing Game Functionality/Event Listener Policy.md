...
| `GlobalDashboardPcSheetSnapshot` | B | — | Dashboard execute-lua: JSON snapshot of five PC seats (stats, Desire, Ambition, session flags, conditions). No Steam gate (no clicker). |
| `GlobalDashboardPcSheetApply` | B+C | — | Dashboard execute-lua: one typed mutation **or a JSON array of mutations** (trackers, dots, disabled, ST badges, Hunger/XP, Desire, Absent/join, ST rolls) then one snapshot. Damage uses V5 overflow (super→agg). Stain add is a no-op while impaired. No Steam gate. Reuses `PCST` / `P` / `Sync.player` / `FSL.setPlayerAbsentFromSession` / `RC.initiateRoll`. |
