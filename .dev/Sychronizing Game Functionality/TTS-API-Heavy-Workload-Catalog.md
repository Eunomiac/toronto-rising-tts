...
| `MusicPlayer.skipForward` | `MusicPlayer` | MODERATE | `asset-load` | `Music Player.md`: may load next clip. | spike | No hot-path skipping. | Debounce explicit command. | set specific clip once | Performance Audit |
...
| `Player.copy` | `Player` | MODERATE | `state-sweep` | Player Instance docs: makes player copy specified object list. | cumulative | Avoid large object arrays in hot path. | Cap object count; explicit command only. | stored GUID list / controlled clone | Reconciler Contract |
...
| `Player.changeColor` | `Player` | MODERATE | `state-sweep` | Player Instance docs: changes player to another color seat. | spike | Never call from implicit hot state update. | Explicit admin/player action; validate target color. | prompt player to choose seat | Reconciler Contract |
