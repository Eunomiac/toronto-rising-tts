...
- There is no table.
- There are no thrones or chairs. `C.Tables["Scatter"].objectsToHide` (also available as `C.Scatter.objectsToHide`) lists objects (chairs, Prince signet and curtain, and anything else you add) that Scatter disables and hides from every player, including the Storyteller. PC pile layout can briefly restore those objects (it treats `HiddenObject` satellites as occupancy stash); `ScatterMode.applyWorldLayout` re-parks the hide list after every rig pass. Leaving Scatter restores their usual visibility, then seat layout places them on the destination table. Restore must keep the layout pose (`keepWorldPose`) so chairs and seat lights are not snapped back to their Scatter hide snapshots. Table C (FACING) also re-runs layout a moment later so live table bounds are read after the wood table is unparked.
...
Scatter Mode computes each occupant’s **reference figurine pose** (position on the correct arc, yaw from the orientation rule below). Bags, sheets, lights, and other seat objects hang off that pose the same way they do on a wood table. **Cameras use the same writer and the same finish as wood tables:** `applyCameraModesForSeats` derives `playerData.cameraAngles` from `C.ReferenceCameraAngles` (look-at **Y** stays at the authored value; only XZ follows the seat), then `M.snapDefaultCamerasAfterLayout` recopies the occupied seat onto the Storyteller and snaps seated players to `default`. Scatter runs that finish immediately after piles move; wood-table scene/table switches still run it under the cover. Scatter does **not** reuse Standard Mode’s circular **table-slot** walk (`numSegments` / `angleSegmentOne` / numbered chairs). Those functions answer a different question.
