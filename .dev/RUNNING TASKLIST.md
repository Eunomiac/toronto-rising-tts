...
_Stack rank for the current cycle (2026-07-04 post **TOR-284** execution model remediation Done + inbox promotions **TOR-285**–**TOR-288**). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._
...
- [ ] **Player HUD missing on first connect:** UIs absent until reload when connected as White/unseated; investigate connect-time sync vs TTS limitation. _(TOR-285)_
...
- [x] **Execution model remediation:** Remove inverted host-execution gating (`isHostClient`, `requireHostForWorldMutation`, join-client bootstrap); keep actor identity + per-client UI; rewrite multiplayer docs/rules. Code: `core/global_script.ttslua`, `sync.ttslua`, `main.ttslua`, domain modules, `lib/util.ttslua`, object stubs — commit `77cac3f`. Author hotseat Save & Play still pending. _(TOR-284)_
