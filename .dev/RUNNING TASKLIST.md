...
**Also in cycle (below top stack):** **TOR-144** (Multiplayer E2E playbook — multiclient pass via **TOR-249** human gate). **TOR-284** (execution model remediation — code Done `77cac3f`; **In Progress** until author hotseat §4.2 sign-off). **TOR-294** (rouse-only broadcast dice row), **TOR-296** (roll broadcast message audit; **blockedBy** **TOR-141** E2E playbooks). **TOR-292** (ST roll dash broadcast persistence), **TOR-290** (stage board Unlock font), **TOR-291** (gitignore bundle-size-gate). **TOR-293** (absent player connect/disconnect presence; **blockedBy** **TOR-144** multiclient E2E). **TOR-286**–**TOR-288** (visibility helper, dice preload, companion toggles). **TOR-95** (play-as-NPC, **blockedBy** **TOR-247**).
...
- [ ] **Roll broadcast message audit:** Catalog broadcast copy by roll type, result class, and exceptions (no difficulty, etc.). _(TOR-296 — `blockedBy` **TOR-141** E2E playbooks)_
