...
- [x] **ST dice drawers — absolute X/Z homes per table:** Dummy `x`/`z`/`rotation` on each `C.Tables[*].objectPositions` ST drawer + matching light (author tunes later); restore prefers table absolute homes after TOR-302 NPC reposition. _(TOR-344)_
...
- [ ] **Intermission theme fade-in from volume 0:** Non-zero `incomingFadeSeconds` for TR_Loop enter. _(TOR-366)_
...
- [ ] **Stage spotlight shortest-angle rotation during lerp:** Fix util + `npc_stage_lerp` light rot bake. _(TOR-369)_
