...
_Stack rank for the current cycle (2026-07-03 post **TOR-226** secret ST rolls author-verified + **TOR-225** display strip / ST tray Done). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-281** — Stage Clear seat activation + library persistence | NPC/scene workflow gap after TOR-250 Done; pairs with TOR-244 library edits |
| 2 | **TOR-244** — Scenes library preview/edit before Apply | Scene library UX; complements TOR-281 live seat persistence |

**Also in cycle (below top stack):** **TOR-144** (Multiplayer E2E playbook — pre-invite checklist shipped in Preparing doc; multiclient pass via **TOR-249** human gate: friend on second machine). **TOR-284** (execution model remediation — Done 2026-07-04). **TOR-95** (play-as-NPC, `blockedBy` **TOR-247** seat decoupling). **ST dice UX cluster Done** (**TOR-224** selective reroll, **TOR-225** sort + tray, **TOR-226** secret rolls). **TOR-222** (clock fast-forward). **Scenes** — **TOR-247** (rotational seat layout). **Projects (human gates):** **TOR-228**–**TOR-231**; **TOR-232** (`blockedBy` TOR-228–231). Dice E2E (**TOR-141** living doc) baseline shipped.
