...
**Also in cycle (below top stack):** **TOR-305** (player dice tray on roll initiate). **TOR-303** (author review roll broadcast phrasing — External Todo). **TOR-144** (Multiplayer E2E playbook — multiclient pass via **TOR-249** human gate). **TOR-284** (execution model remediation — code Done `77cac3f`; **In Progress** until author hotseat probe in [Multiplayer-E2E](E2E%20Playbooks/Multiplayer-E2E.md) passes). **TOR-292** (ST roll dash broadcast persistence), **TOR-291** (gitignore bundle-size-gate). **TOR-293** (absent player connect/disconnect presence; **blockedBy** **TOR-144** multiclient E2E). **TOR-286**–**TOR-288** (visibility helper, companion toggles). **TOR-95** (play-as-NPC, **blockedBy** **TOR-247**).
