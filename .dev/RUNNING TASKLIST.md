...
- [x] **Control token tag contract:** Handler matrix + `pcToken:<Color>` identity; Apply-time occupancy from seat-row tokens (TOR-247); reconcile pins PC token to `tableSlot` chair or Absent park; NPC-only palette/preload via `D.getNpcCharacters()`. _(TOR-236)_
