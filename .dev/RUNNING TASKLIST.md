...
_Stack rank for the current cycle (2026-08-17: Focus batch shipped **TOR-489** / **TOR-486** / **TOR-487** / **TOR-482** / **TOR-488**, pending Save & Play). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / "Deferred this cycle" is paused** (author 2026-06-21) - open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-439** (join-stress re-verify after TOR-444) | Verification gate — remount weight cut needs a multiclient re-smoke |

**Proposed next Focus fills (not auto-applied):** **TOR-81** (centralize light modes), **TOR-168** (Sync.full call-site audit), **TOR-98** (Spotlight phase remaining UX).
...
- [x] **Take Half with Rouse auto-rolls:** Rouse / Obliv-Rouse leftover dice toss automatically (same as right-click Roll). Dice-E2E H2 no longer waits for a human throw. relatedTo **TOR-317**. Pending Save & Play. _(TOR-487)_
...
- [x] **Clear right-click no-op + palette re-snap:** Right-click Clear parks strays, re-snaps on-palette tokens, and always broadcasts ST feedback. relatedTo **TOR-485**. Pending Save & Play. _(TOR-486)_
...
- [x] **Apply Location full transition blindfold:** Live District/Site Apply runs the same staged blindfold as scene Apply, including destination district/site cards. Skybox-only stays on **TOR-478**. relatedTo **TOR-425**. Pending Save & Play. _(TOR-488)_
...
- [x] **Daysleep Refresh gold button:** Years row far-right gold **Refresh**; session-start WP heal (`Phases.runSessionStartWillpowerHeal`). relatedTo **TOR-454**, **TOR-143**. Pending Save & Play. _(TOR-482)_
...
- [x] **Compulsion variant tally:** On first choice from the four master-deck cards, increment `gameState.playerData[playerID].compulsionsTally[Type][1-4]`. relatedTo **TOR-204**. Pending Save & Play. _(TOR-489)_
