...
| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-311** — Scenes panel NPC seat disable clears all seated NPCs | Session-blocking ST/scenes bug — one seat toggle must not vacate every NPC |
| 2 | **TOR-308** — Preload pool dice destroyed instead of returned | Regression after **TOR-287** — pool integrity affects every roll |
| 3 | **TOR-304** — Control board Apply dice-guard retry skip | ST workflow bug — aborted Apply blocks figurine sync after dice cleared |

**Also in cycle (below top stack):** **TOR-307** (startup loose-dice cleanup before table sync). **TOR-306** (Take Half auto-broadcast). **TOR-305** (player dice tray on roll initiate). **TOR-310** (smooth preload spawn + ST Y=8; **blockedBy** **TOR-308**). **TOR-309** (difficulty-0 roll results display). **TOR-312** (narrative roll broadcast successes). **TOR-244** (scenes library preview/edit). **TOR-303** (author review roll broadcast phrasing — External Todo). **TOR-144** (Multiplayer E2E playbook — multiclient pass via **TOR-249** human gate). **TOR-284** (execution model remediation — code Done 77cac3f; **In Progress** until author hotseat probe in [Multiplayer-E2E](E2E%20Playbooks/Multiplayer-E2E.md) passes). **TOR-293** (absent player connect/disconnect presence; **blockedBy** **TOR-144**). **TOR-286**–**TOR-288** (visibility helper, companion toggles). **TOR-95** (play-as-NPC, **blockedBy** **TOR-247**).
...
- [ ] **Startup loose-dice cleanup:** Destroy loose d10 on table + stow trays before startup table sync; preserve d10Preload pool. _(TOR-307)_
- [ ] **Preload pool return regression:** Used dice must return to pool (12/bag); only overflow spawns destroyed on resolution. _(TOR-308)_
- [ ] **Smooth preload-pool spawn + ST Y=8:** Non-teleport claim from pool; ST spawn raised to y=8 before unlock. lockedBy **TOR-308**. _(TOR-310)_
...
- [ ] **Execution model remediation:** Host-execution gates removed; actor identity + per-client UI retained. **Agent slice Done** (code + docs, commit `77cac3f`). **Closes when** author completes the TOR-284 hotseat console probe and regression steps in [Multiplayer-E2E](E2E%20Playbooks/Multiplayer-E2E.md). _(TOR-284)_
