...
- [x] **ST dice drawers — absolute X/Z homes per table:** Dummy `x`/`z`/`rotation` on each `C.Tables[*].objectPositions` ST drawer + matching light (author tunes later); restore always uses those table absolutes after TOR-302 NPC reposition (no live snapshot). _(TOR-344)_
