...
**Also unblocked (not Focus top):** **TOR-89** (PCs map location modal). **TOR-101** (Memoriam LUT/HUD). **TOR-247** (rotational seat layout - unblocks Play-as-NPC). **TOR-495** (spotlight tuner panel, already In Progress). Soft design waits: **TOR-92** / **TOR-99**. **Blocked:** **TOR-320**/ **TOR-321** wait on **TOR-81**; **TOR-330** waits on workshop **TOR-327**; **TOR-95** waits on **TOR-247**. Living docs: **TOR-141**, **TOR-464**. External: **TOR-88**, **TOR-463**, **TOR-454**, **TOR-456**, **TOR-455**, **TOR-303**. **TOR-168** (Sync.full audit) stays in the Focus table even though Linear archived it on 2026-07-03 — it is still Todo with an open tasklist bullet.
...
- [x] **Frenzy overlay disables Rouse bags:** While `hudFrenzy` is on, Rouse and Oblivion-Rouse bags hide; clearing frenzy restores them (Oblivion-Rouse still follows Hunger 5). relatedTo **TOR-203**, **TOR-491**. Pending Save & Play. _(TOR-505)_
...
- [x] **getTracks must not shadow Lua `type()`:** Session Starter `effect` copy used `type(effectName)` while the first argument was named `type`, so catalog load failed (`attempt to call a string value`) and character sheets spammed `GlobalResolveSheetPlayerID`. relatedTo **TOR-497**. Pending Save & Play. _(TOR-503)_
- [x] **Intermission→Play player seat lights:** OutdoorDim apply now also runs `L.reconcileAllPlayers` so `playerLight*` come back on after AdminDark. relatedTo **TOR-497**. Pending Save & Play. _(TOR-504)_
