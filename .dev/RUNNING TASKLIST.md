...
_Stack rank for the current cycle (2026-07-03 post **TOR-225** display strip + ST tray spawn author-verified + **TOR-224** ST selective reroll Done). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._
