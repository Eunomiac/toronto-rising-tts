...
_Stack rank for the current cycle (2026-08-22 late inbox: PAVE cleared seven confirmed rows; **TOR-507** numbered-slot layout already In Progress → Focus #1). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / "Deferred this cycle" is paused** (author 2026-06-21) - open work stays in domain sections; sequence via Linear blockers only._
...
| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-507** (numbered slots + figurine offsets) | Already In Progress: player-positioning Phase 1. |
...
**Also unblocked (not Focus top):** **TOR-89** (PCs map location modal). **TOR-101** (Memoriam LUT/HUD). **TOR-247** (rotational seat layout - unblocks Play-as-NPC; now waits on **TOR-507**). **TOR-495** (spotlight tuner panel, already In Progress). **TOR-168** (Sync.full audit — Linear archived 2026-07-03, still Todo). Soft design waits: **TOR-92** / **TOR-99**. **Blocked:** **TOR-320**/ **TOR-321** wait on **TOR-81**; **TOR-330** waits on workshop **TOR-327**; **TOR-95** waits on **TOR-247**. Living docs: **TOR-141**, **TOR-464**. External: **TOR-88**, **TOR-463**, **TOR-454**, **TOR-456**, **TOR-455**, **TOR-303**.
...
- [ ] **Numbered table slots + figurine offsets (Phase 1):** Occupancy on `sessionScene.seatSlots` (`tableSlot` / `absentFromSession`); Table A dumps → `C.SeatRoleOffsets`; Table B from highest occupied slot; PCs-panel absent toggle; no Scatter. relatedTo **TOR-496**, **TOR-247**. _(TOR-507)_
