...
- [x] **Seated NPC active scale 53:** Set figurine scale 53 at table seat; restore `npcs_data` `figurine.scale` on unseat/stage/preload. _(TOR-223)_
...
- [ ] **Play as NPC:** Control-board `pc_control_token` seat activate/deactivate; play-as-NPC swaps sheet by tag; `sessionScene.npcRoleOverride` / `seatSlots`; lighting exception per Scene Constructor spec ([Scene Constructor Overview](Scene%20Constructor/Scene%20Constructor%20Overview.md)). _(TOR-95)_
...
- [ ] **Non-Host onLoad guard audit:** Document TTS client script authority; early-return Host-only paths in `onLoad`/bootstrap (NPC sync, moves, setup). _(TOR-221)_
