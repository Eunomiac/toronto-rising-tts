...
- [ ] **Rouse-only roll broadcast — main dice row:** Pure Rouse/Obliv-Rouse checks show dice in `rollRes_die_*`, not corner rouse panel. _(TOR-294)_
- [ ] **Roll broadcast message audit:** Catalog broadcast copy by roll type, result class, and exceptions (no difficulty, etc.). _(TOR-296)_
- [ ] **Seated NPC ST roll — reposition drawer, spawn circle, tray light:** When ST rolls for an NPC at an active table seat, move the chosen ST dice drawer (via `DICE_DRAWER_STORYTELLER_NPC<#>` anchors), spawn circle, and matching dice light in front of that seat; restore default ST drawer/light positions on roll clear. _(TOR-302)_
...
- [x] **Non-Host onLoad guard audit:** Document TTS client script authority; early-return Host-only paths in `onLoad`/bootstrap (NPC sync, moves, setup). _(TOR-221)_ — *superseded 2026-07-04 by **TOR-284** (execution model remediation; host-execution gates removed).*
- [ ] **Execution model remediation:** Remove inverted host-execution gating (`isHostClient`, `requireHostForWorldMutation`, join-client bootstrap); keep actor identity + per-client UI; rewrite multiplayer docs/rules. **Agent slice Done** (code + docs, commit `77cac3f`). **Closes when** author completes §4.1–4.2 hotseat Save & Play per [Remediation Plan](Multiplayer%20Functionality/Execution%20Model%20Correction%20%E2%80%94%20Remediation%20Plan.md) Definition of Done. _(TOR-284)_
