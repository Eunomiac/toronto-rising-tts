...
_Stack rank for the current cycle (2026-07-15 — inbox: Global HUD first-load + coterie/Status promotions; **TOR-387** stake dots shipped). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-384** — Global HUD missing on first save load | Session-blocking; object XmlUI OK, Global.xml absent until reload |
| 2 | **TOR-385** — sheetDisplay:false Status overlays + Stats/Projects | Immediate; overlays + list path |
| 3 | **TOR-386** — Domain ratings chasse/lien/portillon/haven in PE/Stats | Immediate; PE listing gap |
| 4 | **TOR-329** — TTS API heavy-workload audit | Performance / hitch inventory; waits on #1 |
...
- [ ] **Global HUD missing on first save load:** Object XmlUI OK; Global.xml absent until reload; Scripting UI still shows XML on first load. relatedTo **TOR-285**, **TOR-381**. _(TOR-384)_
...
- [x] **Map panel — north pins clipped:** Top buffer + `LowerCenter` pins in `panel_map_core_defaults.xml`. _(TOR-379)_
...
- [ ] **sheetDisplay:false Status:** Temp/disabled overlays on page 3; list in Stats + Project Editor Advantages. _(TOR-385)_
- [ ] **Domain ratings in Stats + Project Editor:** Top-level `chasse` / `lien` / `portillon` / `haven` stakeable + listed when Coterie selected. _(TOR-386)_
