...
_Stack rank for the current cycle (2026-07-15 — **TOR-375** closed with CSHEET setXml exception; parallel **TOR-365** / **TOR-293** / **TOR-328**). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-365** — End scene location wiped from library (investigate) | Root-cause first; no speculative harden |
| 2 | **TOR-293** — Absent player presence override (connect/disconnect) | Disconnected = scene-absent, incl. no-scene |
| 3 | **TOR-328** — POST_ROLL confirm-only auto-broadcast | Completes TOR-306 single-button proceed |
| 4 | **TOR-329** — TTS API heavy-workload audit | Performance / hitch inventory |
