...
- [ ] **ST roll panel selective reroll:** Clickable dice images on ST dashboard; highlight + Reroll button; map to physical dice; no ST reroll caps (hunger OK, repeat until Confirm). _(TOR-224)_
...
- [ ] **Storyteller NPC gameboard (Phase A):** `STAGE_BOARD` + `CONTROL_BOARD`, tokens, markers, configurable `CONTROL_BOARD_SNAP` grid (~160 snaps), Apply/Clear wired; `placements` v3 + `Sync.npcs`. Phase B sub-issues: **TOR-236** (token tags), **TOR-237** (Read/Lock/Load), **TOR-238** (hover stage figurine spotlight — not game-phase Spotlight). Phase C panel retirement Done (TOR-181). _(TOR-169)_ — workshop objects + GUIDs in TTS save required; Save & Play to confirm snap geometry/rotation
- [ ] **Control token tag contract:** Handler matrix + workshop spawn for `npc_control_token` vs `pc_control_token`; seat-row PC semantics feed play-as-NPC. _(TOR-236)_
- [ ] **Control board Read / Lock / Load:** Wire XmlUI toolbar stubs (`layoutLock` toggle, Read/Load semantics). _(TOR-237)_
...
- [ ] **Map pins — last active location + timestamp:** Pin syncs to last known location only when current scene time is later; update on scene/time/location change and PC activate/deactivate. _(TOR-245 — `blockedBy` TOR-151)_
...
- [ ] **Play as NPC:** Control-board `pc_control_token` seat activate/deactivate; play-as-NPC swaps sheet by tag; `sessionScene.npcRoleOverride` / `seatSlots`; lighting exception per Scene Constructor spec ([Scene Constructor Overview](Scene%20Constructor/Scene%20Constructor%20Overview.md)). `blockedBy` **TOR-236** (token contract). _(TOR-95)_
...
- [ ] **Multiplayer E2E playbook:** Pre-invite solo-suite checklist + multi-client test plan (sub-issue of TOR-141). **`blockedBy` TOR-248** (author multi-client TTS workflow — External Todo). _(TOR-144)_
- [ ] **Multi-client TTS session workflow (human gate):** Author establishes repeatable Host + join-client setup for multiplayer verification; document in `.dev/E2E Playbooks/`. Blocks TOR-144. _(TOR-248 — External Todo)_
- [x] **Non-Host onLoad guard audit:** Document TTS client script authority; early-return Host-only paths in `onLoad`/bootstrap (NPC sync, moves, setup). _(TOR-221)_
