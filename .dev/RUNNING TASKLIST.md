...
| 2 | **TOR-168** (Sync.full call-site audit) | Low-risk sync hygiene; helps prevent future regressions. |
...
- [x] **Idle Oblivion-Rouse bag right-click auto-rolls a 1-die check:** Same shortcut as **TOR-490** on the Oblivion-Rouse bag (Purple). Hunger 5 still locked out. relatedTo **TOR-490**. Pending Save & Play. _(TOR-491)_
...
- [x] **Take Half with Rouse auto-rolls:** Rouse / Obliv-Rouse leftover dice toss automatically (same as right-click Roll). After settle, sole Confirm auto-broadcasts (TOR-328). Dice-E2E H2/H2b assert `noActive` (no Confirm click). relatedTo **TOR-317**. Inbox follow-up 2026-08-18. Pending re-test. _(TOR-487)_
...
- [x] **Clear right-click no-op + palette re-snap:** Right-click Clear parks strays, re-snaps on-palette tokens, and always broadcasts ST feedback. Inbox follow-up: one tagged-token scan instead of two. relatedTo **TOR-485**. Pending re-test (lag). _(TOR-486)_
...
- [x] **Daysleep Refresh gold button:** Years row far-right gold **Refresh**; session-start WP heal (`Phases.runSessionStartWillpowerHeal`). Works without a live scene; overlay hides after ~6s. relatedTo **TOR-454**, **TOR-143**. Pending re-test. _(TOR-482)_
