...
_Stack rank for the current cycle (2026-07-05 post **TOR-294** + **TOR-302** Done: rouse-only broadcast + seated NPC ST drawer reposition). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-244** — Scenes library preview/edit before Apply | Top Focus after TOR-294/TOR-302 shipped; unlocks safer library editing workflow |
| 2 | **TOR-287** — Dice preload pool | Performance polish — instant spawn without placeholder flash |
| 3 | **TOR-296** — Roll broadcast message audit | Catalog broadcast copy by roll type / result class |
...
- [x] **Rouse-only roll broadcast — main dice row:** Pure Rouse/Obliv-Rouse checks show dice in `rollRes_die_*`, not corner rouse panel. _(TOR-294)_
