...
| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-398** — Global blindfold flickers during load + Intermission→Play | Session-blocking overlay race; related to TOR-363 / TOR-361 |
| 2 | **TOR-395** — Close entire ST toolbar when panel closes (except panel switch) | ST workflow polish; extends TOR-364 close-on-Advance |
| 3 | **TOR-392** — TTS API heavy-workload agent/docs guardrails (phase 4) | Unblocked by TOR-391 remediation Done |
...
- [ ] **Close entire ST toolbar when panel closes:** Collapse `storytellerToolbarBody` on panel close / `closeAllPanels`; exception = panel switch. _(TOR-395)_
- [ ] **Global blindfold flickers on load + Intermission→Play:** Competing writers / XML refresh during those windows. relatedTo **TOR-363**, **TOR-361**. _(TOR-398)_
...
- [ ] **TTS API heavy-workload agent guardrails (phase 4):** Update docs/rules so agents avoid new hot-path misuse. `blockedBy` **TOR-391**. _(TOR-392)_
