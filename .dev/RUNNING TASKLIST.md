...
**Also in cycle (below top stack):** **TOR-144** (Multiplayer E2E playbook — pre-invite checklist shipped in Preparing doc; multiclient pass via **TOR-249** human gate: friend on second machine). **TOR-284** (execution model remediation — code Done `77cac3f`; **In Progress** until author hotseat §4.2). **TOR-286**–**TOR-288** (inbox: visibility helper, dice preload, companion toggles). **TOR-95** (play-as-NPC, `blockedBy` **TOR-247** seat decoupling). **ST dice UX cluster Done** (**TOR-224** selective reroll, **TOR-225** sort + tray, **TOR-226** secret rolls). **TOR-222** (clock fast-forward). **Scenes** — **TOR-247** (rotational seat layout). **Projects (human gates):** **TOR-228**–**TOR-231**; **TOR-232** (`blockedBy` TOR-228–231). Dice E2E (**TOR-141** living doc) baseline shipped.
