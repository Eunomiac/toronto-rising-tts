...
- [ ] **Startup loose-dice cleanup:** Destroy loose d10 on table + stow trays before startup table sync; preserve d10Preload pool. `cleanupLooseDiceBeforeTableSync` before `R.SyncTable` on load. _(TOR-307 — code shipped, author verify)_
...
- [ ] **Open player dice tray on roll initiate:** Drawer opens at PRE_ROLL (`GlobalOpenPlayerDiceTrayForRoll` on `openRoll` / skip-setup `initiateRoll`), not on first bag die spawn. _(TOR-305 — code shipped, author verify)_
- [ ] **Take Half auto-broadcast:** Pure Take Half confirms/broadcasts without extra Confirm; general single-button auto-proceed on roll panel. _(TOR-306 — code shipped, author verify)_
