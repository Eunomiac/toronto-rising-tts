...
_Stack rank for the current cycle (2026-07-13 — **TOR-144** / **TOR-249** multiclient E2E passed). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-365** — End scene location wiped from library (investigate) | Capture-only residual vs TOR-145 |
| 2 | **TOR-293** — Absent player presence override (connect/disconnect) | Unblocked after TOR-144; High Players & Connection |
