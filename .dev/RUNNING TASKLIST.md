...
- [ ] **Global XmlUI remount weight:** Image stacks → `setAttribute(image)`; seat-local chrome tint (A); shared panels + visibility unions; map sidebars Option B (shared + stack visibility); `U.setVisibleTo`/`showTo`/`hideFrom`. Phases 0–3 done; next = shared refs. See [Global-UI-Image-Stacks](HUDs%20%26%20Overlays/Global-UI-Image-Stacks.md). _(TOR-444)_
