...
_Stack rank for the current cycle (2026-08-06 evening `/tr-inbox`: PAVE cleared verified items; capture empty; Focus re-stack). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / "Deferred this cycle" is paused** (author 2026-06-21) - open work stays in domain sections; sequence via Linear blockers only._
...
| 3 | **TOR-418** (clock lerp silent no-op) | High scenes bug stuck In Progress - validate/Alert before close |
| 4 | **TOR-419** (group flip family/face/Y/stage light) | High NPC bug stuck In Progress - gameboard polish after TOR-484/485 |
...
- [x] **Spotlight?End transition blindfold:** Staged HUDBF transition on Spotlight exit to End (same shape as End scene). relatedTo **TOR-98**, **TOR-143**. Pending Save & Play. _(TOR-459)_
