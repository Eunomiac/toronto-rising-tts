...
- [x] **Intermission→Play session-start explode:** Same Play-enter step as the Music C sting starts the stacked cover explode (cover, then still-text / wavering-art pairs, then session number/title). relatedTo **TOR-497**, **TOR-514**, **TOR-515**. Pending Save & Play. _(TOR-516)_
- [x] **Session-start overture starts at full volume:** Music C one-shot sets gain to catalog volume before play, so the opening drum is not eaten by the looping silent-arm delay. relatedTo **TOR-497**. Pending Save & Play. _(TOR-515)_
...
- [x] **Global blindfold stacked panel:** Show/hide/active and HUD canary target `overlay_globalBlindfold_panel` (stacked splash Images inside). Overlay Alpha tints child Images. Pending Save & Play. relatedTo **TOR-398**, **TOR-384**, **TOR-442**. _(TOR-514)_
