...
**Also in cycle (below top stack):** **TOR-344** (ST dice drawer absolute X/Z per table). **TOR-141** (E2E playbooks living doc). **TOR-329** (TTS API heavy-workload audit). **TOR-286** (centralize `setInvisibleTo`). **TOR-303** (author review roll broadcast phrasing — External Todo). **TOR-144** / **TOR-249** (Multiplayer E2E — human gate; blockers **TOR-143** + **TOR-345** Done; docs include auto-seat + phase connect). **TOR-293** (absent player connect/disconnect presence; **blockedBy** **TOR-144**). **TOR-288** (companion toggles). **TOR-95** (play-as-NPC, **blockedBy** **TOR-247**). **TOR-330** (Fomorach shapeshift toggle; **blockedBy** **TOR-327** workshop stat deltas). **TOR-98** (Spotlight turn UX — scaffolding in TOR-143).
