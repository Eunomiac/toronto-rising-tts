...
_Stack rank for the current cycle (2026-07-16 — **TOR-389** scene NPC seat sync shipped; **TOR-204** Compulsions in flight). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-204** — Compulsions deck (revised RunSequence flow) | In Progress; DRAWN/SELECTED + lights + one-at-a-time |
...
- [ ] **Global HUD missing on first save load:** Object XmlUI OK; Global.xml absent until reload; Scripting UI still shows XML on first load. relatedTo **TOR-285**, **TOR-381**. Investigate: `DEBUG.showGlobalHudCanaries()` — no speculative remount. _(TOR-384)_
