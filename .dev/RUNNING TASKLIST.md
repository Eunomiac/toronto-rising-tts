...
_Stack rank for the current cycle (2026-07-08 post `/tr-inbox`: **TOR-314** (scenes location modals) shipped; **TOR-315** (NPC stage light rescale) promoted). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-307** — Startup loose-dice + tray restore | Author verify ST workshop restore (`73bfbf6`); code shipped |
