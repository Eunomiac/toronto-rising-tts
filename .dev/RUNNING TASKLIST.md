...
_Stack rank for the current cycle (2026-06-23, post `/tr-inbox` triage). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-144** — Multiplayer E2E playbook | TOR-221 (bootstrap guards) Done; `blockedBy` cleared — ready for multi-client verification |
| 2 | **TOR-83** — Site modal overlap fix | Scenes panel click target bug; Linear **Quick Fix**; independent small win |
| 3 | **TOR-236** — NPC vs PC control token contract | New Phase B sub-issue; prerequisite for **TOR-95** (play-as-NPC / `pc_control_token`) |

**Also in cycle (below top stack):** **NPC gameboard Phase B** — **TOR-237** (Read/Lock/Load toolbar), **TOR-238** (hover token → stage figurine spotlight preview; **not** Spotlight game phase TOR-98). **TOR-95** (play-as-NPC, `blockedBy` TOR-236). **ST dice UX** — **TOR-224** / **TOR-225** / **TOR-226**. **TOR-222** (clock fast-forward). **Scenes inbox promotions** — **TOR-244** (library preview/edit), **TOR-245** (map pin timestamps, `blockedBy` TOR-151), **TOR-247** (rotational seat layout). **TOR-169** umbrella (In Progress; workshop save verification). **Projects (human gates):** **TOR-228**–**TOR-231**; **TOR-232** (`blockedBy` TOR-228–231). Dice E2E (**TOR-141** living doc) baseline shipped.
