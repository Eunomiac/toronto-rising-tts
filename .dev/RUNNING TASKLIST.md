...
- [x] **Stage NPC token Y on scene transition:** Token Y slightly inside board mesh during scene transition — nudge up so visible on drop. _(TOR-393)_
...
- [x] **Scenes panel UI trim + 3-column library:** Remove instructional copy; scene buttons show name + status only; three-column grid. _(TOR-139)_
...
- [x] **Phases sessionNum InputField after End:** set `text` attribute + re-sync on Phases open (inactive setValue ignored). _(TOR-397)_
- [x] **Close entire ST toolbar when panel closes:** Collapse `storytellerToolbarBody` on panel close / `closeAllPanels`; exception = panel switch. _(TOR-395)_
- [x] **Global blindfold flickers on load + Intermission→Play:** Idempotent show/hide (no FadeIn when already up); relatedTo **TOR-363**, **TOR-361**. _(TOR-398)_
...
- [x] **Sound panel UI trim + larger text:** Remove excess instructional copy; increase Text element font size. _(TOR-140)_
