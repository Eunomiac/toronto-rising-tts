...
_Stack rank for the current cycle (2026-08-03 — INBOX Immediate → **TOR-453** / **TOR-452** Focus; Quick Fixes **TOR-450** / **TOR-451** shipped). **Precedence** = Focus stack + Linear **`blockedBy`** (not Linear priority). **TOR-141 (E2E playbooks)** is a living doc (In Progress, not Focus stack). **Back-burner / “Deferred this cycle” is paused** (author 2026-06-21) — open work stays in domain sections; sequence via Linear blockers only._

| # | Issue | Why now |
| --- | --- | --- |
| 1 | **TOR-453** (Scene Time → present day when no scene selected) | INBOX Immediate; wrong Jan 1 2026 defaults |
| 2 | **TOR-452** (Compulsion card + companion state with tray open/close) | INBOX Immediate; live-roll table choreography |
| 3 | **TOR-448** (ST roll randomize pop — unlock → 0.15s → clicker-seat toss) | Match PC auto-roll order; pending Save & Play |
| 4 | **TOR-444** (Global XmlUI remount weight — Phase 7 join smoke) | Join Reload UI / multiclient verify after Phase 6 |
| 5 | **TOR-439** (join-stress minimal XmlUI spike verify) | Pairs with TOR-444 Phase 7 |
| 6 | **TOR-436** (blindfold parent off-screen) | Save & Play pending |
...
- [ ] **ST roll tray/light before dice + player-like randomize pop:** Tray/light ordering shipped; ST dice spawn unlocked at tray Y=2.04 then unlock → 0.15s → `randomize(clickerSeat)`. Pending Save & Play. _(TOR-448)_
- [ ] **Compulsion card + companion with tray open/close:** On tray open move selected Compulsion to `COMPULSION_CARD_SELECTED_LIVEROLL_<COLOR>` and companion figurine state 1→2 (when enabled); reverse on close. relatedTo **TOR-305**, **TOR-204**. _(TOR-452)_
...
- [x] **Remove FirstPerson force on blindfold/scene transition:** Keep TOR-368 default lookAt; reverse TOR-443 `setCameraMode("FirstPerson")`. Pending Save & Play. _(TOR-446)_
...
- [ ] **Scene Time when no scene selected:** Show `presentDayClock` (green) instead of Jan 1 2026 defaults; blank if no present-day set. relatedTo **TOR-449**. _(TOR-453)_
...
- [x] **Debug timestamp on panel open:** Re-sync `debug_lastLoadClock` when expanding `adminControlsBody` (inactive setValue ignored). Pending Save & Play. _(TOR-406)_
...
- [x] **Defer transition work until blindfold-down finishes:** `U.chain` waits ~2.5s (slide + card FadeIns) before fade-out / heavy reconcile so animations are not hitch-frozen. Pending Save & Play. _(TOR-434)_
...
- [x] **Timing API merge:** `U.stagger` / `U.chain` / `U.await` (+ `U.cancel`); `Time.time` clocks; unique chain waiters; hard-cut old names. Policy + handoff updated. Pending Save & Play smoke. _(TOR-438)_
...
- [ ] **Global XmlUI remount weight:** ~7435→~1808 Images (Phase 6 core author-confirmed 2026-08-03: nestedless+Court+blindfold+location+map districts+sidebars B+hunger). Optional: location full share; pip meters on `tor-444-court-pip-meters`. **Next: Phase 7** join Reload UI / multiclient smoke. See [Global-UI-Image-Stacks](HUDs%20%26%20Overlays/Global-UI-Image-Stacks.md). _(TOR-444)_
...
- [x] **Pink Tarot deck spawn Y:** `TAROT_DECK_PINK.on.position.y` 12 → 7.7 (semi-lock physics). Pending Save & Play. _(TOR-411)_
- [x] **Shuffle Tarot deck on activate:** `deck.shuffle()` when turning ON in `TarotToggle.applyTarotStateNow`. Pending Save & Play. _(TOR-447)_
...
- [x] **Join-client HUD/UI missing (TTS):** Ported EmoJuice deferred full-UI resync fallback + seat-layout miss skip; multiclient Phase A re-smoke still author-owned. _(TOR-381)_
