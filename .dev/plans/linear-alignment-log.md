...
| 2026-06-25 | DONE | TOR-147 | Soundscape fade on blindfold down — staged scene transition `HUDBF.runStagedTransition` (`U.RunSequence`: fade-out → heavy reconcile → fade-in → settle/lift); `Soundscape.fadeOutTransitionAmbient` (emitter-only, weather outgoing-vs-incoming policy) + `beginTransitionFadeWindow`/`previewIncomingWeather`; `Scenes.buildSoundscapeContextForSession`/`applyNoSceneSoundscape` + `applyDefaultNoSceneEnvironment({skipSoundscape})`; Apply + End scene wired; solo verify pending author, multiclient unverified (TOR-144) |
