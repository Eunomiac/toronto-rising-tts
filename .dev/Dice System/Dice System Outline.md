...
**Physical dice (Black only):** pool counts are configured in state (grid strips, ST bag clicks) during SETUP/PRE_ROLL; bags do not spawn dice until **ROLL**. `HUD_rollRollButton` drives **`U.chain`** (`maxWait` 10s per drawer gate, same threshold as `STD.isRaisedForRelease`): (1) smooth-open ST drawer (`STR.activateDrawerForSlot`), then wait until raised; (2) spawn locked dice at bags (`spawnBlackPoolLocked`), wait `ST_ROLL_SPAWN_SETTLE_SEC` (0.75s); (3) wait until drawer raised; (4) instant `setPosition` onto tray arc (`moveBlackStagedDiceToTray`), wait `ST_ROLL_TRAY_SETTLE_SEC` (2s locked — PC auto-roll usually happens long after manual assemble); (5) wait until drawer raised; (6) `RC.startRolling` + bag `releaseDice` unlock; (7) 0.15s free-physics window; (8) simultaneous `Object.randomize(clickerSeat)`. ST tray placement uses instant positioning; drawer motion stays smooth.
