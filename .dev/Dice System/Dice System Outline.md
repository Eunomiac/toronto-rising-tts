...
**Physical dice (Black only):** pool counts are configured in state (grid strips, ST bag clicks) during SETUP/PRE_ROLL; bags do not spawn dice until **ROLL**. `HUD_rollRollButton` drives **`U.RunSequenceWithOptions`** (`maxWait` 10s per drawer gate, same threshold as `STD.isRaisedForRelease`): (1) smooth-open ST drawer (`STR.activateDrawerForSlot`), then wait until raised; (2) spawn locked dice at bags (`spawnBlackPoolLocked`), wait 0.5s; (3) wait until drawer raised; (4) instant `setPosition` onto tray arc (`moveBlackStagedDiceToTray`), wait 1s (locked hover); (5) wait until drawer raised; (6) `RC.startRolling` + bag `releaseDice` unlock + simultaneous `Object.randomize("Black")` from Global (`randomizeBlackTrayDiceSimultaneous`). ST tray placement uses instant positioning; drawer motion stays smooth.
