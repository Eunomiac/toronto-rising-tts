...
**Physical dice (Black only):** pool counts are configured in state (grid strips, ST bag clicks) during SETUP/PRE_ROLL; bags do not spawn dice until **ROLL**. `HUD_rollRollButton` drives **`U.chain`** (`maxWait` 10s per drawer gate, same threshold as `STD.isRaisedForRelease`): (1) smooth-open ST drawer (`STR.activateDrawerForSlot`), then wait until raised; (2) spawn unlocked dice on tray surface (`STS.STAGING_BASE_Y` = **2.04**), wait 0.5s; (3) wait until drawer raised; (4) tray ellipse ring layout (`moveBlackStagedDiceToTray`, stay unlocked), wait 1s; (5) wait until drawer raised; (6) `RC.startRolling` + bag `releaseDice` (clear staging; already unlocked); (7) 0.15s; (8) simultaneous `Object.randomize(clickerSeat)`. ST dice are **unlocked** for the whole sequence so the toss matches PC auto-roll physics.
