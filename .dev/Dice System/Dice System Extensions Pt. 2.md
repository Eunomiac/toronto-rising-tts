...
| Player **right-clicks** `DICEBAG_ROUSE` with no active roll | Shortcut (**TOR-490**): initiate a 1-die Rouse check, Open it (skip Storyteller approval), and auto-toss (same as right-click Roll). After settle, existing Confirm / auto-apply rules still apply. May overlap other idle-bag auto-Rouses only (**TOR-493**); blocked if any live player roll is not that shortcut. Same seat cannot start a second while one is still live or cleaning up. |
| Player **right-clicks** `DICEBAG_OBLIVROUSE` with no active roll | Shortcut (**TOR-491**): same as idle Rouse right-click, but a 1-die Oblivion Rouse check. Hunger 5 stays locked out (same as left-click). After settle, existing Confirm / Hunger-or-Stain / auto-apply rules still apply. Counts as the same shortcut as idle Rouse for the **TOR-493** overlap rule. |
...
**Manual rouse vs Oblivion-Rouse are mutually exclusive** (`pool.rouse` blocks Obliv bag; `pool.oblivRouse` blocks manual Rouse bag). **`bloodSurgeRouse` does not block Obliv** and does not disable the Obliv bag. **At Hunger 4, Blood Surge and manual `pool.rouse` are mutually exclusive** on compound standard/Discipline rolls (TOR-331). **At Hunger 5, Blood Surge is blocked**; forced dedicated Rouse rolls still allowed (TOR-203).
...
- **Blood Surge activation:** `RC.activateBloodSurge` uses `EffectiveStats.forSeat` → `bloodPotencyDerived().bloodSurge` and fresh `ctx.rollPolicy()` for `bloodSurgeDiceMultiplier`. **Hunger 4:** cannot combine Blood Surge with manual `pool.rouse` on one roll (TOR-331). **Hunger 5:** Blood Surge blocked; dedicated `C.RollType.ROUSE` still allowed. Bag visibility: when `pool.oblivRouse > 0`, manual Rouse bag hidden; when surge active or `pool.bloodSurgeRouse > 0`, manual Rouse bag hidden; at Hunger 5, Hunger bag hidden for surge rolls.
- **Blood Surge undo:** Hunger bag right-click → last BS Rouse triggers `RC.deactivateBloodSurge` → `GlobalDestroyBloodSurgeDice` (BS Rouse + surge-count main-pool dice preferring Normal first; clear leftover `bloodSurgePool` tags).
- **Take Half + Rouse:** if the pool has Rouse/Oblivion-Rouse dice, Take Half applies only to normal+hunger (`DK.nonRouseVampirePoolTotal`), destroys those dice, unlocks remaining Rouse dice and **auto-randomizes** them (`GlobalReleaseRouseDiceForActiveRoll`, TOR-487), then `onDieSettledSignal` debounce → `recalculate` merges strips when all expected rouse dice are on-table with readable faces (locks them on merge) before POST_ROLL. If the only remaining action is Confirm, the roll auto-broadcasts (TOR-328).
