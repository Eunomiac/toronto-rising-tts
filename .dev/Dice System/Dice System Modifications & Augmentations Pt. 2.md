...
- **Blood Surge activation:** `RC.activateBloodSurge` uses `EffectiveStats.forSeat` → `bloodPotencyDerived().bloodSurge` and fresh `ctx.rollPolicy()` for `bloodSurgeDiceMultiplier`. Oblivion-Rouse in pool does **not** block surge (only the manual Rouse bag is hidden). Blood Surge rouse dice use `pool.bloodSurgeRouse`, not `pool.rouse`.
