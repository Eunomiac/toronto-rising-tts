...
| `conditions` | string[] | Optional. Registry condition ids with **`kind = "scene"`** (same pattern as `C.Districts[*].conditions` for location ids). Applied to **present** PCs via `Conditions.reconcileHostedForSession` for the scene duration. Stat and roll effects use registry channels (`statChanges`, `roll`, etc.). **`rollDefaults` is removed** — import fails if present. |
