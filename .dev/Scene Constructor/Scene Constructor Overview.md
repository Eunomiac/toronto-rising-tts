...
| `siteKey` | string \| null | Site within district. Live location drives skybox via `Scenes.reconcileSkyboxFromState` (`C.Sites[siteKey].skybox` catalog key → `C.Skyboxes[key].url`, or random `C.GenericSkyboxes`) unless `skyboxOverride` is set. |
| `skyboxOverride` | string \| null | Optional. Catalog key into `C.Skyboxes`, or `"Generic"` (force random `C.GenericSkyboxes`, ignoring site default). When set, `Scenes.reconcileSkyboxFromState` uses that path instead of the site skybox. Omit / `null` / blank → no override (site default). **URLs are rejected on import.** |
...
1. **Blindfold down** — close Scenes panel; arm one random blindfold variant (+ destination District/Site cards when both keys resolve) on the shared parent Panel, then one `UI.show` on `playerHud_overlay_blindfold`, then **re-arm children** (parent show resets XML defaults — TOR-431 / TOR-425 / TOR-444). Cards FadeIn with XML delays (district ~4s, site ~4.5s) so players see blindfold art first. End scene / table-only / missing location keep cards `color=Clear` / empty image so they stay invisible.
2. **Lead-in (~4.0s)** — hold until parent FadeIn finishes and the district-card FadeIn window starts (`HUDBF.BLINDFOLD_DOWN_COMPLETE_SEC` = district `showAnimationDelay` 4.0s). Same clock when cards are Clear (End scene). No heavy work yet (TOR-434 / TOR-476).
3. **Fade-out (~1s)** — `Soundscape.fadeOutTransitionAmbient` fades BGM + location + weather toward silence (emitter-only; no `gameState` intent change). Weather compares **outgoing vs incoming** (preview from the target bundle): different track → full fade-out; same track, different volume → duck to the **lower** volume (hold without restart, TOR-136). Overlaps district/site card reveal.
4. **Site-card gate (~0.75s after fade-out)** — wait until site FadeIn completes (~5.5s wall clock: delay 4.5 + duration 1) plus a **0.25s pad** before heavy work (~5.75s), so the card is fully visible before Sync/camera hitch risk.
5. **Heavy work** — default camera snap for seated players (TOR-368; under full cover), then commit a matching Control Board THERE draft before cloning the row. If `gameState.controlBoard.lockNextSceneApply` is armed, merge the outgoing live stage placements, occupied NPC seats, and PC presence over the destination participants, persist the merged participants back to the destination library row, then write live state, switch table, hosted reconcile, and `Sync.full({ skipSoundscape = true })`. Clear the one-shot lock only after this activation succeeds. New soundscape is **not** applied here.
6. **Fade-in under cover (~2s)** — immediately after work (no silent workSettle first), `Scenes.applyActiveSceneSoundscapeFromSession()` runs inside `Soundscape.beginTransitionFadeWindow` under the blindfold (TOR-476; ~2s `TRANSITION_FADE_IN_SEC`).
7. **Settle + lift** — remaining post-work down-time (`settleDelaySec - workSettleSec - fadeInSec`), then one parent `UI.hide` lifts the blindfold (+ child card FadeOuts); child defaults restore after ~3s (TOR-431). Audio is already full at lift.

(Standalone table toggles still use `runTransition`, which also waits for blindfold-down before work; the legacy single-burst `runTransitionAfterLeadIn` remains for callers that apply everything at once. Non-staged settle paths still snap default cameras via `HUDBF.scheduleEnd` when settle delay > 0.)
...
- **End Scene** — narrative end: notify players, **flush live clock** onto the mirroring library row, then **`SceneLibrary.detachLiveTableFromLibraryMirror()`** (stop `receivesLiveWrites`, clear `lastAppliedKey`), then **clear `activeKey`** so the ended scene is deselected (TOR-449 — preview is for scenes about to enter, not the one just ended), apply default no-scene via staged blindfold (TOR-147). Closes the Scenes panel.
