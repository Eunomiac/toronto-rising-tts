...
1. **Blindfold down** — close Scenes panel, show one random blindfold variant; slide-in is 1s. When the destination bundle has both `districtKey` and `siteKey`, also show `overlay_district_card_display_*` / `overlay_site_card_display_*` with matching Custom UI images at the same `UI.show` moment (TOR-425). End scene / table-only / missing location leave those Images `active=false`.
...
5. **Lift + fade-in (~2s, concurrent)** — at the **end** of the settle, `Scenes.applyActiveSceneSoundscapeFromSession()` runs inside `Soundscape.beginTransitionFadeWindow` **as the blindfold rises**, so the new scene audio fades in alongside the visual reveal (TOR-273; ~2s `TRANSITION_FADE_IN_SEC`). Blindfolds (and destination location cards, when shown) lift once via `scheduleEnd(0)`.
