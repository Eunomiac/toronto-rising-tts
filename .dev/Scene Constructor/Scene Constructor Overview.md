...
5. **Fade-in under cover (~2s)** — `Scenes.applyActiveSceneSoundscapeFromSession()` runs inside `Soundscape.beginTransitionFadeWindow` **under the blindfold** so new ambient ramps to full before lift (TOR-476; ~2s `TRANSITION_FADE_IN_SEC`).
6. **Settle + lift** — remaining post-work down-time (`settleDelaySec - workSettleSec - fadeInSec`), then one parent `UI.hide` lifts the blindfold (+ child card FadeOuts); child defaults restore after ~3s (TOR-431). Audio is already full at lift.
