...
| `rollDefaults` | object | Optional. Top-level keys: **`all`** or any **`C.RollType`** value (`standard`, `simpleCheck`, `rouse`, …). Each value is an object of `active.rollOptions` keys (`takeHalf`, `wpReroll`, `hungerDice`, `bestialNull`, `crits`, `canRerollHunger`, `numberOfRerolls`, `numberOfDiceRerolled`). Unknown type or option keys **fail import validation** (modal stays open). |
