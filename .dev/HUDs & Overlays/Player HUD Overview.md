...
- **Runtime `setAttribute`:** `chasse`, `lien`, `portillon`, `haven` → `dot_on_<key>_<1–5>_<Color>` on Prince's Court page 2; **blank trait slots** (`blank: true`, `active: true`) → title/flavor/rules/dots on pre-baked ids (`pc_p{page}_{bg|merit|flaw}_{index}_<Color>_…`) via `lib/coterie_collect.ttslua`.
