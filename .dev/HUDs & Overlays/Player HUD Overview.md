...
- **Pin coordinates:** `C.Sites[*].offsetXY` are absolute UI offsets on `hud_map_draggable_container` (**height 1600**, center stack **2100** in `panel_map_core_defaults.xml`). Do not enlarge that stack for clipping headroom — it desyncs pins from the map art (**TOR-388** / **TOR-379**). Prefer softer viewport sink + `LowerCenter` tip alignment.
