...
Shared XmlUI overlay (TOR-444) toggled from the right sidebar (`toggle_PrincesCourt_*`). Analogous to CSHEET facing-page pairs: each Court **panel** is one double-page image (left + right halves), not a physical object.

| `princesCourtPage` | Facing book pages | Nav buttons |
| --- | --- | --- |
| 1 | 1–2 | next only |
| 2 | 3–4 | prev + next |
...
- **Runtime `setAttribute`:** `chasse`, `lien`, `portillon`, `haven` → shared `dot_on_<key>_<1–5>`; project stakes → shared `dot_project_*`; **blank trait slots** (`blank: true`, `active: true`) → shared pre-baked ids (`pc_p{page}_{bg|merit|flaw}_{index}_…`) via `lib/coterie_collect.ttslua`.
- **Build-time trait columns:** Backgrounds/merits/flaws for coterie (page 2 / `page_1_right`), domain (page 3 / `page_2_left`), and haven (page 4 / `page_2_right`) use the same 3-column greedy packing and trait-box markup as CSHEET page 3 (`lib/trait_sheet_xml.ttslua`). `npm run build:xml` / `build:full` runs `node .dev/scripts/princes_court_trait_placeholders.js` (reads `Coterie.json`, hydrates blank slots via `lib/coterie_hydrate.ttslua` mirror, expands column placeholders) before `xml_color_template_generator.js`. **Blank slots** (`blank: true`, `active: false`) are **not** in the Google Sheet JSON — `lib/coterie_hydrate.ttslua` appends three per trait array after decode; rebuild from JSON resets sheet-backed rows (in-session blank content is state-only until rebuild).
- **Section dividers (build-time):** `##IF @@HAS_*@@##` in `panel_right_sidebar_referenceLayer.xml` strips entire trait sections (divider + columns) when `Coterie.json` has no real entries for that array (`buildTraitSectionDividerFlags` in `.dev/scripts/lib/coterie_hydrate.js`). Coterie page 2 still always shows backgrounds; merits/flaws dividers only. Domain/haven pages wrap backgrounds, merits, and flaws. **Domain Claims** divider is always present.

**Page 1 tracker dots:** Five columns (one per seat color) mirror CSHEET page-1 health / willpower / humanity boxes and hunger slots. XML ids use `box_{layer}_{stat}_{index}_{ColumnSeat}` on the shared tree (e.g. `box_on_health_3_Purple`). **Positioning:** tracker `Image`s use `class="box box_standard"` with `offsetXY` authored for `rectAlignment="MiddleLeft"` — the reference layer `<Defaults>` overrides global `defaults_classes.xml` `box` (`UpperLeft`, for CSHEET object sheets) back to `MiddleLeft` + `active="true"`. Reconcile: `HUDP.reconcilePrincesCourtTrackersForHudSeat` when the reference is open on page 1; `HUDP.reconcilePrincesCourtTrackersForPanelSeat` from `PCST.refreshCharacterSheetsForColor` (gated when no HUD has court page 1 open). Collect still emits clear-then-fill; apply **coalesces** to net `active`/`image` and **fingerprints** the coalesced result so unchanged columns skip XmlUI (TOR-377). RT clock ticks must not call full `playerHud` refresh. Logic: `PSC.collectPrincesCourtTrackerUpdates` in `lib/pc_sheet_collect.ttslua` (wraps page-1 `collectSheetImageUpdates` + hunger).
...
- **Prince's Court Reference** — Three double-page spreads (`refPanel_PrincesCourt_page1`–`page3` = facing pages 1–2 / 3–4 / 5–6). Page 3 has prev only. Sidebar opens on last `princesCourtPage`. Visibility uses Lua-side audience unions (`applyPrincesCourtPageVisibility`).
...
- **Pin coordinates:** `C.Sites[*].offsetXY` are absolute UI offsets on `hud_map_draggable_container` (**height 1600**, center stack **2100** in `panel_map_core_defaults.xml`). Do not enlarge that stack for clipping headroom — it desyncs pins from the map art (**TOR-388** / **TOR-379**). Pin Image class must keep default **`height="100%"`** (only override `width="300"`) and explicit **`rectAlignment="MiddleCenter"`** (TTS default; kept explicit) so the pin rect stays map-tall and offsets match authored geometry. Do **not** set `preserveAspect` on pins — it collapses height to the texture and drops every pin by ~half the map. Do **not** set `LowerCenter`. **North pin clipping:** baseline viewport sink in XML (`hud_map_center_container` `offsetXY` **0 -400**); when the draggable map is panned north (`y < 0`), `core/hud_player.ttslua` shifts `playerHud_mapViewport_*` further down (up to **+120** px at full north pan) without changing the draggable pin parent. Pins keep `ignoreLayout="true"` — TTS turns a Panel into a LayoutGroup if it gains `padding`, which would reflow absolute pins.
