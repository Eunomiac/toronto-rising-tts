...
U.chain({
  function()
    ...
    if S.getStateVal("sceneLibrary", "activeKey") ~= nil then error("[Scenes E2E] FAIL - activeKey should clear after End scene (TOR-449; deselect ended live scene)") end
...
U.chain({
  ...
  function()
    printHeader("[HUMAN] With a scene live, use Browse sites to pick a different site and click Apply location + soundscape. Expect the full scene-transition blindfold with destination district/site cards (not the short skybox-only fade).", 3)
  end
})
```
