...
- **Session-start overture (`playSessionIntro` on Music C):** Trigger one-shots with `fadeSeconds` 0 and a non-zero `fromVolume` set emitter gain to the catalog volume **before** `playTriggerEffect` and re-assert it in the same tick. They do **not** use the looping silent-arm delay, which ate the opening drum (TOR-515).
...
- **Outdoor rain particles (TOR-498 / TOR-508):** The table object `G.GUIDS.PARTICLES_WEATHER_RAIN` is a Custom AssetBundle looping list (author slots 1–13 → TTS indexes **0–12**), **not** a soundscape audio channel. `Soundscape.setRainLayer` / `setWindLayer` / `setThunderEnabled` / `setIndoors` (and silent-site / `stopAll`) apply the matching index in the same tick as weather intent. Indoor, silent site, or no rain → index 0 (None). Thunderstorm rows require live thunder **and** heavy rain. Skip `playLoopingEffect` when `getLoopingEffectIndex` already matches. Load bootstrap may try index 0 before `getObjectFromGUID` works — miss is silent (same cold window as audio emitters). Snow is out of scope (**TOR-463**).
