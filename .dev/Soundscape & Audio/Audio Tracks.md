...
One-shot Intermission→Play lead-in on emitter `SOUNDSCAPE_MUSIC_C` (`musicC`). Not featured music and not a Main playlist track. Play enter picks `introKey` from `C.SessionStartAnimationData[sessionNum]` (missing index uses `[1]`; fallback catalog key `TR_SessionStart`). Live Trigger Effect name for that default clip is **`Session Starter`**. TTS `playTriggerEffect` indexes start at **0**, so object UI slot **2** is Lua index **1**. Duration and cover timing follow that session's `songDuration` (lerp path vs `C.SessionStartBaseDuration`; attribute path scales so the track and splash end together — TOR-567). Volume **1**, no fade-in. Lua sets emitter gain to that volume **before** `playTriggerEffect` (TOR-515); do not reintroduce the looping silent-arm delay on this clip. Play enter finishes OutdoorDim lighting behind the cover first (TOR-535), waits `C.SessionStartPlayEnterSettleSec` so that work can finish (TOR-536), then either (default) runs the TEST BED splash sequence: TR_Loop starts fading at splash start over the scaled song lead-in and reaches silence when this sting starts on pair 1, or (Phases **Lerp explode** on) kicks the lerp cover explode, waits `C.SessionStartIntroDelaySec`, then fires this sting (TOR-533 / TOR-534 / TOR-535 / TOR-559 / TOR-567). Prefer **Preload Audio Data** so the first session start does not hitch.
