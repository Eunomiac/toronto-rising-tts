...
One-shot Intermission→Play lead-in on emitter `SOUNDSCAPE_MUSIC_C` (`musicC`). Not featured music and not a Main playlist track. Play enter picks `introKey` from `C.SessionStartAnimationData[sessionNum]` (missing index uses `[1]`; fallback catalog key `TR_SessionStart`). Live Trigger Effect name for that default clip is **`Session Starter`**. TTS `playTriggerEffect` indexes start at **0**, so object UI slot **2** is Lua index **1**. Duration and explode timing follow that session's `songDuration` vs `C.SessionStartBaseDuration` (71s). Volume **1**, no fade-in. Lua sets emitter gain to that volume **before** `playTriggerEffect` (TOR-515); do not reintroduce the looping silent-arm delay on this clip. Play enter kicks the cover explode **before** this sting, then waits `C.SessionStartIntroDelaySec` (0.5s) so the opening drum hits with the first visible scale (TOR-533 / TOR-534). Prefer **Preload Audio Data** so the first session start does not hitch.
