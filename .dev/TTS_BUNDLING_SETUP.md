...
- **Not the same as color templates**: Top-level `ui/.templates/*.xml` files with `<!-- TARGET: ui/... -->` are processed by `xml_color_template_generator.js` into shipped per-color UI files. Nested folders such as `ui/.templates/csheet/` are **embed-only** (no `TARGET` line). **`ui/.templates/roll/`** is a **build-time composer** (partials + `dash_body.xml` → `ui/shared/roll_dash_generated.xml` via `npm run roll-dashboard:generate`); not embedded in Lua and not expanded by the color generator. **`ui/.templates/storyteller/panel_debug_light.xml`** is the same kind of composer (partial `debug_button_light.xml` × 400 → `ui/storyteller/panel_debug_light.xml` via `npm run debug-light-panel:generate`); run it **before** `ui-global-xml:embed` so the Storyteller HUD Include sees the generated pool. Lua still fills those slots at runtime (`UI.setClass` / `U.setAttributes` text); it does not remount Global XML.
