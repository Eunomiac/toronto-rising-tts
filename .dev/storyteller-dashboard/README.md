...
3. **PCs** — live play sheet: left player rail (session / connect / ST rolls) plus a two-page spread. Page 1 is the character sheet (dots, trackers, Desire). Page 2 is reserved empty space. Reads TTS through execute-lua (`GlobalDashboardPcSheetSnapshot`). Clicks update the dashboard immediately and queue `GlobalDashboardPcSheetApply`; clicks that pile up while TTS is busy go in one JSON array. **Claim Port** (red) seizes 39998 from TTS Tools and retries the live sheet. **Release Port** (green) drops the dashboard’s hold so TTS Tools can use the port again. Tabletop Simulator itself is left running.
4. **Lua** — Execute Code into a live TTS session (same External Editor hook as the TTS Tools extension). Only one process can listen on a given bind of port 39998. Use **Claim Port** / **Release Port** on the PCs tab to take or give back the port. To stop a leftover dashboard `node` on that port without closing Cursor: Run Task **FREE TTS EDITOR PORT (39998)**, or `npm run tts-bridge:free-port` from the repo root.
