...
3. **PCs** — live play sheet: left player rail (session / connect / ST rolls) plus a two-page spread. Page 1 is the character sheet (dots, trackers, Desire). Page 2 is reserved empty space. Reads and writes TTS through execute-lua (`GlobalDashboardPcSheetSnapshot` / `Apply`). Until Save & Play loads those Globals, the tab shows fixture stats. **Clear port** (top right) stops anything on 39998 — including the TTS Tools extension — then retries the live connection. Tabletop Simulator itself is left running.
4. **Lua** — Execute Code into a live TTS session (same External Editor hook as the TTS Tools extension). Disable the extension first; only one process can listen on a given bind of port 39998. To stop a leftover dashboard `node` on that port without closing Cursor: Run Task **FREE TTS EDITOR PORT (39998)**, or `npm run tts-bridge:free-port` from the repo root.
