...
3. **PCs** — live play sheet: left player rail (session / connect / ST rolls) plus a two-page spread. Page 1 is the character sheet (dots, trackers, Desire). Page 2 is reserved empty space. Reads and writes TTS through execute-lua (`GlobalDashboardPcSheetSnapshot` / `Apply`). Until Save & Play loads those Globals, the tab shows fixture stats. Disable the TTS Tools extension first.
