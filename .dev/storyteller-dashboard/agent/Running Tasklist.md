...
✔️ SD-PCs **JSON** debug button. Opens raw `playerData` for the seat; patch + **Apply** deep-merges into `gameState.playerData` with **no** XP normalize / defaults / validateState (`mergePlayerData`).
