...
✔️ SD-PCs async apply queue. PCs-tab clicks paint immediately (`applyLocal`); TTS `GlobalDashboardPcSheetApply` runs on a serial background queue so a slow execute-lua round-trip cannot freeze Hunger/XP. Author ⚠️ on the live-sheet PAVE row (2026-09-21).
