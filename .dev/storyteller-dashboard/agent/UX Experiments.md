...
- **Names:** Pack-aware sides plus authored starting offsets from `tokenNameOffsets.json` (the Fill-and-Lock dump). Hovering a name in Debug outlines that token in red so clustered labels are easier to match. Board tokens and their labels stack by height: lower on the parchment paints in front. That stack stays inside the board overlay so pop-up modals always sit above tokens.
- **Lit tokens:** Gold disc plus a halo that pulses on the token (`inset: 0%`).
- **District picker:** Toronto map at 800×1000, no header bar or inner scroll; floating **X** on the image. Pins are invisible (zero-alpha text, still hit-sized), gold translucent fill on hover, gold border and no fill when selected. Click a pin to choose that district, then the Sites modal opens. Closing with the floating **X** does not open Sites. A location’s Lua `conditions` list turns those chips on in the Conditions widget, and leaving the location turns them off again unless the new place also applies them.
