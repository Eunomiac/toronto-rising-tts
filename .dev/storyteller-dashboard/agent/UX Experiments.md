...
- **Names:** Pack-aware sides plus authored starting offsets from `tokenNameOffsets.json` (the Fill-and-Lock dump). Hovering a name in Debug outlines that token in red so clustered labels are easier to match. Board tokens and their labels stack by height: lower on the parchment paints in front.
