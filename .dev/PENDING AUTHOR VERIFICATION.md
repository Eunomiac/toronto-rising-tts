...
#### ⚠️ TOR-487 — Take Half with Rouse dice auto-rolls
...
**Corrections:** The roll should auto-confirm, not wait for the player to click Confirm. After clicking "Take Half" and automatically rolling the Rouse die, the player is presented with a prompt that has a single button -- "CONFIRM".  This violates a policy we established earlier, where players should never be prompted with a single-button confirmation prompt (or any other "single choice" prompt). Expected behavior: Because there is only one button, the confirmation popup should be skipped and the roll should automatically resolve.
---
...
#### ❌⚠️ TOR-482 — Daysleep Refresh gold button
...
**Correction:** The Refresh button should not require a live scene to work -- it should work even without a live scene.

**Verification Failure:** After clicking the Refresh button, the modal remains on-screen and does not automatically disappear after a few seconds (like other broadcast messages).
...
**Verified:** The functionality works as described.

**Verification Failure:** The action is quite a bit more laggy than I would have expected. Please check and confirm there is no extra work being done on a right-click Clear. Don't take heroic measures to squeeze out more performance -- only if there is an obvious inefficiency should you implement a correction; otherwise, mark this issue as completely verified.
