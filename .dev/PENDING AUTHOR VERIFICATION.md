...
1. **Enter Scatter:** Apply a library scene whose JSON has `"placementMode": "scatter"`, or from the Host console run `lua require("lib.rotational-seat-layout").SetTableTo("Scatter")`. The control board should switch to the Scatter parchment, every wood table and throne/chair should vanish, and polar snap points on the board should be gone. The floor, table plinth, rain emitter, and stage board should sit on X/Z `{0, 0}` (their heights should not change). The two main lights should sit at Z `0`.
