...
- **Linear:** Toronto Rising team (`TOR-*`); 12 domain projects; epics TOR-30–43; ~114 issues after 2026-05-22 alignment. Always-on agent rule: `.cursor/rules/toronto-rising-linear.mdc`. Planned work lives in `.dev/RUNNING TASKLIST.md` (every bullet `_(TOR-XX)_`); audit trail `.dev/plans/linear-alignment-log.md`. Linear MCP: `plugin-linear-linear`.
