...
1b. `.dev/PENDING AUTHOR VERIFICATION.md` — **read/skim** if the Focus item or named work still needs author TTS confirmation, or if the author is about to playtest. **Do not** process **✅** / **❌** / **⚠️** marks here — that is **`/tr-inbox` only** (see `.dev/PENDING AUTHOR VERIFICATION.agent.md`). If this session **ships** TTS-observable code, **add an Outstanding row in the same session**. If marks are present and would change what you should work on, tell the author to run `/tr-inbox` (or ask them to confirm you may process marks now).
