...
| Situation | Use |
|-----------|-----|
| Starting, continuing, or closing any feature/fix (always) | **Linear** (`.cursor/rules/toronto-rising-linear.mdc`): check `TOR-*` issues, update status/comments, keep RUNNING TASKLIST in sync; shipped TTS work → **Awaiting Author Review**, verified work → **Fully Complete** by default or **Complete (KEEP)** sparingly; if the issue cap blocks a create, ask the author to clear all **Fully Complete** issues. |
...
| After testing / capture sessions | **`/tr-inbox`** ([`.cursor/skills/tr-inbox/SKILL.md`](../../../.cursor/skills/tr-inbox/SKILL.md)): process INBOX + sync Focus; **`blockedBy`** for sequencing (anti-gridlock: star pattern, short lists); priority separate from Focus rank; **no back-burner deferral** → then **`/tr-start`** in a new chat |
