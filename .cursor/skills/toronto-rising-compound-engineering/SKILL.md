...
This project has **long-running** Tabletop Simulator systems (dice, roll pipeline, rotational layout, lighting, workshop objects). Use Compound Engineering **proactively** in these situations:
...
| Starting a fresh agent session | **`/tr-start`** ([`.cursor/commands/tr-start.md`](../../.cursor/commands/tr-start.md)): Focus + mandatory policy reads + architecture non-negotiables; **commit without asking**; after Linear **In Progress**, prompt author to rename chat (`TOR-XXX — title`) — agents cannot rename Cursor chats |
| After testing / capture sessions | **`/tr-inbox`** ([`.cursor/commands/tr-inbox.md`](../../.cursor/commands/tr-inbox.md)): process INBOX + sync Focus; **`blockedBy`** for sequencing (anti-gridlock: star pattern, short lists); priority separate from Focus rank; **no back-burner deferral** → then **`/tr-start`** in a new chat |
| Resuming work, debugging something touched before, or avoiding contradicting earlier design | **Session history** (`ce-sessions` skill): search prior agent sessions for this workspace before large edits. |
| Multi-step or cross-module change (several files, unclear sequencing) | **Planning** (`ce-plan` / `ce:work` skills): short plan or structured execution, then implement; create or link Linear issues before coding. |
| Feature or fix is “done” and non-trivial; before merge/PR | **Structured review** (`ce:review` skill): tiered review appropriate to risk/size. |
| Shipping: commit message + PR body should match repo standards | **Git / PR** skills (`git-commit`, `git-commit-push-pr`, `ce-pr-description`): align with `.cursorrules` (Linear refs, clear narrative). |
