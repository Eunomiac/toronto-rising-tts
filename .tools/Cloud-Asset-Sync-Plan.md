...
| `memoriamNpcFigurines` / `memoriamNpcTokenFronts` / `memoriamNpcTokenBacks` | `NPC Cutouts/Memoriam/…` (`mem_<stem>`, `mem_BACK` shared reverse, `tokenFront_mem_<stem>`, `tokenBack_mem_<stem>`) | Figurine keys keep `mem_`; tokens match other NPC categories (`tokenFront_` / `tokenBack_` first) | Constants: `npc.name` = `mem_<stem>` → figurine front `mem_<stem>`, `figurine.back` from `mem_BACK`, tokens `tokenFront_mem_<stem>` / `tokenBack_mem_<stem>` |
