...
__bundle_register("lib.guids", function(require, _LOADED, __bundle_register, __bundle_modules)
-- lib/guids.ttslua
-- GUIDs Library
...
-- Storyteller gets an entry from C.StorytellerID. Other players are added when they select a seat.
...
function S.GetDefaultGameState()
    local playerData = {}
    -- Storyteller is the only hard-coded player; others are added dynamically on seat selection
    playerData[C.StorytellerID] = {
        hunger = 0,
        color = "Black"
    }
...
function S.mergePlayerData(statePlayerData)
    ...
    -- Second pass: add players in state but not in C.PlayerData (new players awaiting GM setup)
    if statePlayerData then
        for playerID, dynamicData in pairs(statePlayerData) do
            if merged[playerID] == nil and type(dynamicData) == "table" then
                merged[playerID] = U.clone(dynamicData)
                -- Add minimal placeholder static fields so lookups don't fail
                merged[playerID].playerName = merged[playerID].playerName or ("Player " .. tostring(playerID):sub(-6))
                merged[playerID].charName = merged[playerID].charName or "Unassigned"
            end
        end
    end
    ...
    if gameState.playerData then
        ...
        for playerID, playerData in pairs(gameState.playerData) do
            ...
            elseif type(playerID) ~= "string" and type(playerID) ~= "number" then
                ...
                local dynamicFields = {"hunger", "color"}
...
--- Gets the storage ID for a Player object (steam_id as string, or steam_name for spectators)
-- Used as the key for gameState.playerData. Storyteller is normalized to C.StorytellerID.
-- @param player Player TTS Player object
-- @return string|nil Player ID string, or nil if player is invalid
-- @usage local id = S.getStorageID(Player.Brown)
function S.getStorageID(player)
    if player == nil or not U.isPlayer(player) then
        return nil
    end
    -- Spectators/local players may have "Player N" as steam_name; use that if no real steam_id
    if string.match(player.steam_name or " ", "^Player ") then
        return player.steam_name
    end
    local steamID = tostring(player.steam_id)
    -- Normalize Storyteller to hard-coded ID
    if steamID == C.StorytellerID then
        return C.StorytellerID
    end
    return steamID
end

--- Initializes a new player's state entry when they first select a seat
-- Creates minimal dynamic data and notifies the GM to set up character details.
-- @param playerID string The player's steam_id (storage ID)
-- @param color string The seat color they selected (e.g. "Brown")
-- @param steamName string Optional. Player's Steam display name for the broadcast message
function S.initNewPlayer(playerID, color, steamName)
    if gameState == nil or gameState.playerData == nil then
        print("[State] initNewPlayer: gameState not initialized")
        return
    end
    if gameState.playerData[playerID] ~= nil then
        return -- Already has data
    end

    -- Create minimal dynamic state (same structure as GetDefaultGameState)
    gameState.playerData[playerID] = {
        hunger = 0,
        color = color
    }

    local displayName = steamName or playerID
    local msg = string.format(
        "New player joined: %s (ID: %s) selected %s. Please add their character data to C.PlayerData in lib/constants.ttslua.",
        displayName, playerID, color
    )
    U.AlertGM(msg, Color(1, 0.85, 0))
    print("[State] " .. msg)
end

--- Gets the player ID for a player object or color string
-- For Player objects: returns getStorageID (steam_id or C.StorytellerID).
-- For color strings: finds the player ID that has that color in gameState.playerData.
...
function S.getPlayerID(playerRef)
    ...
    -- Player object: use getStorageID
    if type(playerRef) == "userdata" and U.isPlayer(playerRef) then
        return S.getStorageID(playerRef)
    end

    -- String: could be color or already a player ID
    if type(playerRef) == "string" then
        -- If it's a known player ID (in playerData), return it
        if gameState and gameState.playerData and gameState.playerData[playerRef] then
            return playerRef
        end
        -- If it's Storyteller ID, return it
        if playerRef == C.StorytellerID then
            return C.StorytellerID
        end
        -- Assume it's a color: find who has this color in playerData
        if gameState and gameState.playerData then
            for playerID, pData in pairs(gameState.playerData) do
                if type(pData) == "table" and pData.color == playerRef then
                    return playerID
                end
            end
        end
        -- Fallback: check C.PlayerData for color match (for Storyteller or pre-configured players)
        for pid, pData in pairs(C.PlayerData) do
            if pData.color == playerRef then
                return pid
            end
        end
    end
...
--     * 180° = -Z axis (toward players, in front of target)
--     * 270° = -X axis (to the left)
--     * If center is a GameObject, angle is relative to the object's local frame
...
function U.XYZToCylindrical(pos, center, centerYaw)
	pos = Vector(pos)
	-- Handle Object center references (use local frame when center is an Object)
	if U.isGameObject(center) then
		local localPos = center.positionToLocal(pos)
		local dx = localPos.x
		local dz = localPos.z
		local radius = math.sqrt(dx * dx + dz * dz)
		local angle = math.atan2(dx, dz) * (180 / math.pi)  -- Convert to degrees
		local height = localPos.y
...
--- Resolves a GameObject from an object reference or GUID string
-- Returns the object if already a GameObject, or looks it up by GUID.
-- @param ref Object|string Reference object or GUID string
-- @param isSilent boolean If true, returns nil on failure instead of throwing
-- @return Object|nil Resolved GameObject or nil
-- @usage local obj = U.getObject("abc123")
-- @usage local obj = U.getObject(someObj)
function U.getObject(ref, isSilent)
	if U.isGameObject(ref) then return ref end
	if U.Type(ref) == "string" then
		local obj = getObjectFromGUID(ref)
		if obj then return obj end
		if not isSilent then
			U.error("U.getObject", "No object found for GUID", ref)
		end
		return nil
	end
	if U.Type(ref) == "table" and ref.guid ~= nil then
		local obj = getObjectFromGUID(ref.guid)
		if obj then return obj end
	end
	if not isSilent then
		U.error("U.getObject", "Invalid object reference", ref)
	end
	return nil
end
...
local function cylindricalToXYZ(cylindrical, center, centerYaw)
	if U.isGameObject(center) then
		...
		local localPos = Vector(
			cylindrical.radius * math.sin(angleRad),
			cylindrical.height,
			cylindrical.radius * math.cos(angleRad)
		)
		return center.positionToWorld(localPos)
	end
	center = Vector(center)
	local angleRad = math.rad(cylindrical.angle)
	return Vector(
...
local function sphericalToXYZ(spherical, center, centerYaw, centerPitch)
	if U.isGameObject(center) then
		...
		local localPos = Vector(
			horizontalRadius * math.sin(angleRad),
			spherical.radius * math.cos(angle2Rad),
			horizontalRadius * math.cos(angleRad)
		)
		return center.positionToWorld(localPos)
	end
	center = Vector(center)
...
-- @param data table PositionOrientationData
-- @return table {pos=Vector, obj=Object|nil}
local function resolveCenterData(data)
	if data.center == nil then
		return {pos = Vector(0, 0, 0), obj = nil}
	end
	if U.isGameObject(data.center) then
		return {pos = data.center.getPosition(), obj = data.center}
	end
	return {pos = Vector(data.center), obj = nil}
end
...
function U.GetEasedPath(start, endVal, duration, ease, easeIntensity, coordinateSystem)
	...
	local startCenter = startCenterData.obj or startCenterData.pos
	local endCenter = endCenterData.obj or endCenterData.pos
	...
	local function resolveCoordData(position, coordSystem, center, centerYaw, centerPitch)
		...
		if U.isGameObject(position) then
			...
			if coordSystem == "cylindrical" then
				return U.XYZToCylindrical(pos, center)
			elseif coordSystem == "spherical" then
				return U.XYZToSpherical(pos, center)
			end
			return nil
		end
		if coordSystem == "cylindrical" then
			...
			elseif U.Type(position) == "table" and position.x ~= nil and position.y ~= nil and position.z ~= nil then
				return U.XYZToCylindrical(position, center)
			end
		elseif coordSystem == "spherical" then
			if U.isInstance(position, Vector) then
			...
			elseif U.Type(position) == "table" and position.radius ~= nil and position.angle ~= nil then
				return position
			elseif U.Type(position) == "table" and position.x ~= nil and position.y ~= nil and position.z ~= nil then
				return U.XYZToSpherical(position, center)
			end
		end
		return nil
	end
	...
	_G[coroutineName] = function()
		for i = 0, frames do
			...
			if startPos ~= nil and endPos ~= nil then
				...
				if coordinateSystem == "cylindrical" then
					local startCyl = resolveCoordData(start.position, "cylindrical", startCenter, nil, nil) or U.XYZToCylindrical(startPos, startCenter)
					local endCyl = resolveCoordData(endVal.position, "cylindrical", endCenter, nil, nil) or U.XYZToCylindrical(endPos, endCenter)
				...
				elseif coordinateSystem == "spherical" then
					local startSph = resolveCoordData(start.position, "spherical", startCenter, nil, nil) or U.XYZToSpherical(startPos, startCenter)
					local endSph = resolveCoordData(endVal.position, "spherical", endCenter, nil, nil) or U.XYZToSpherical(endPos, endCenter)
			...
			-- Handle orientation (lock to target object if provided)
			if start.orientation ~= nil or endVal.orientation ~= nil then
				...
				if U.isGameObject(endVal.orientation) then
					targetObj = endVal.orientation
				elseif U.isGameObject(start.orientation) then
					targetObj = start.orientation
				end

				if targetObj ~= nil then
					local success, targetPos = pcall(function() return targetObj.getPosition() end)
					if success and targetPos then
						step.orientation = calculateLookAtRotation(currentPos, targetPos)
					else
						step.orientation = Vector(0, 0, 0)
					end
				else
					local startRot = start.orientation ~= nil and Vector(start.orientation) or nil
					local endRot = endVal.orientation ~= nil and Vector(endVal.orientation) or nil
					if startRot ~= nil and endRot ~= nil then
						step.orientation = lerpRotation(startRot, endRot, tEased)
					elseif startRot ~= nil then
						step.orientation = startRot
...
function DEBUG.testZones()
    ...
    table.insert(sequenceFunctions, function()
        broadcast("🗺️  Starting Zone Management Test", {0.5, 0.8, 1})
        return 2.0
    end)

    -- Step 2: Test zone state retrieval
    table.insert(sequenceFunctions, function()
        broadcast("📊 Checking initial zone state...", {0.8, 0.8, 1})
    ...
    table.insert(sequenceFunctions, function()
        broadcast("✅ Testing Zone Activation", {0, 1, 0})
        Z.activateZones()
        Wait.time(function()
            local zonesLocked = S.getStateVal("zones", "allLocked")
    ...
    table.insert(sequenceFunctions, function()
        broadcast("❌ Testing Zone Deactivation", {1, 0, 0})
        Z.deactivateZones()
        Wait.time(function()
            local zonesLocked = S.getStateVal("zones", "allLocked")
    ...
    table.insert(sequenceFunctions, function()
        broadcast("👁️  Testing Show Zones (zones should move above table)", {1, 1, 0})
        Z.showZones()
    ...
    table.insert(sequenceFunctions, function()
        broadcast("🙈 Testing Hide Zones (zones should move below table)", {0.5, 0.5, 0.5})
        Z.hideZones()
    ...
    table.insert(sequenceFunctions, function()
        broadcast("🔄 Re-activating zones for event testing...", {0.8, 0.8, 1})
    ...
    table.insert(sequenceFunctions, function()
        broadcast("🔍 Testing Zone Object Queries", {0.5, 0.8, 1})
    ...
    table.insert(sequenceFunctions, function()
        broadcast("📥 Testing Zone Events - Object Enter", {1, 0.8, 0})
        broadcast("   → Please place an object INTO a zone now", {1, 1, 0.5})
        broadcast("   → Wait for confirmation message...", {0.8, 0.8, 1})
    ...
    table.insert(sequenceFunctions, function()
        broadcast("📤 Testing Zone Events - Object Leave", {1, 0.8, 0})
        broadcast("   → Please REMOVE an object FROM a zone now", {1, 1, 0.5})
        broadcast("   → Wait for confirmation message...", {0.8, 0.8, 1})
    ...
    table.insert(sequenceFunctions, function()
        broadcast("💾 Testing Zone State Persistence", {0.8, 0.8, 1})
        local testZone = getTestZone()
        if testZone then
    ...
    table.insert(sequenceFunctions, function()
        broadcast("✅ Zone Management Test Complete!", {0, 1, 0})
        print("\n" .. string.rep("-", 60))
  ...
  -- Test configuration constants
	...
	local TEST_PARAMETERS = {
		ORBITING_LIGHT = {
			...
			endVal = {
				position = {radius = {mult = 2}, angle = "+720", angle2 = 20},  -- Radius doubles, angle relative, angle2 absolute
	...
	local function runParallelLerps(lerps, objects, objToMove, defaultDuration, defaultEase)
		...
		for _, lerpDef in ipairs(lerps) do
			...
			if lerpDef.target ~= nil then
				targetObj = objects[lerpDef.target] or U.getObject(lerpDef.target, true) or lerpDef.target
			end
			targetObj = U.getObject(targetObj, true) or targetObj
			U.Val("runParallelLerps", targetObj, targetObj ~= nil, "Parallel lerp target not found")
			...
			if setFunc == nil and lerpDef.setter ~= nil and targetObj ~= nil then
				...
				if U.Type(method) == "function" then
					setFunc = function(val, obj) method(obj, val) end
				end
			end
			...
			local duration = U.Lerp(function(val)
				if targetObj == nil then
					return nil
				end
				if setFunc == nil then
				...
				return setFunc(val, targetObj)
			end, startVal, endVal, lerpDuration, isRotationLerp, lerpEasing)
	...
	local function createTestFromParameters(testName, objects)
		...
		local lerpFunction = function()
			...
			if totalDuration > 0 then
				...
				broadcast("▶️ " .. testDisplayName .. " Running: " .. totalDuration .. "s", {0.8, 0.5, 1})
	...
	if orbitingInit and orbitingOrientation and orbitingLerp then
		...
		table.insert(animationSequence, pauseStep("⏸️ Orbiting Light complete - inspect TEST_LIGHT, then continue."))
	end
...
function DEBUG.runTests()
    ...
    local function runSingleTest(testName, testFunc)
        broadcast("🧪 Running: " .. testName, {0.8, 0.8, 1})
        print("\n" .. string.rep("=", 60))
        ...
        if not success then
            print("❌ ERROR running " .. testName .. ": " .. tostring(result))
    ...
    table.insert(testSequence, function()
        broadcast("🚀 Starting All Tests Suite", {1, 1, 0.5})
        return 2.0
    end)
    ...
    table.insert(testSequence, function()
        broadcast("✅ All Tests Complete!", {0, 1, 0})
        print("\n" .. string.rep("=", 60))
...
--- Debug helper: Move an object to a world-angle position around table center
-- Positions an object at a given distance and world angle around the table center.
-- Uses U.rotateAroundPoint to keep angle conventions consistent.
-- @param obj Object|table|userdata TTS object to move (GameObject)
-- @param distance number Distance from center in world units
-- @param angle number World angle in degrees (0 = +Z, 90 = +X, 180 = -Z, 270 = -X)
-- @param center Vector|table Optional center position (default: table center at object's current Y)
-- @usage lua DEBUG.tableRotateTo(obj, 30, 180)
function DEBUG.tableRotateTo(obj, distance, angle, center)
    local resolvedObj = U.getObject(obj, true) or obj
    U.Val("DEBUG.tableRotateTo", resolvedObj, U.isGameObject(resolvedObj), "obj must be a valid GameObject or GUID")
    U.Val("DEBUG.tableRotateTo", distance, U.Type(distance) == "number", "distance must be a number")
    U.Val("DEBUG.tableRotateTo", angle, U.Type(angle) == "number", "angle must be a number")

    local objPos = resolvedObj.getPosition()
    local centerPos
    if center ~= nil then
        centerPos = Vector(center)
    else
        centerPos = Vector(0, objPos.y, 0)
    end

    local targetPos = U.rotateAroundPoint(centerPos, distance, angle, centerPos.y)
    resolvedObj.setPosition(targetPos)
    print("DEBUG.tableRotateTo: Moved " .. resolvedObj.getName() .. " to " .. tostring(targetPos))
end
...
tableRotateTo = DEBUG.tableRotateTo
...
--[[
    Handle player color/seat selection
    Triggered when a player chooses a seat color (Brown, Orange, Red, Pink).
    Extracts their steam_id and ensures they have a state entry. New players get
    a minimal entry and the GM is notified to set up their character details.

    @param color string The color the player selected. "Grey" = disconnect (ignored).
]]
function M.onPlayerChangeColor(color)
    -- Ignore disconnects (TTS passes "Grey" when a player disconnects)
    if color == "Grey" then
        return
    end

    -- Only act when a player selects a defined seat color (not Black/Storyteller, not spectator)
    ...
    local playerID = S.getStorageID(player)
    if playerID == nil then
        return
    end

    -- Check if they already have state data
    if gameState and gameState.playerData and gameState.playerData[playerID] then
        -- Update their color in case they switched seats
        gameState.playerData[playerID].color = color
        return
    end

    -- New player: initialize state and notify GM
    S.initNewPlayer(playerID, color, player.steam_name)
end
